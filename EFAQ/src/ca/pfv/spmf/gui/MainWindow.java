...
public class MainWindow extends JFrame {
	...
	public static String		SPMF_VERSION		= "0.95";
	// current input file
	...
	public static void main(String[] args) {
		...
		// the command line.
		if (args.length != 0) {
			processCommandLineArguments(args); // process command line
												// arguments.
		}
		else {
	...
	public MainWindow() {
		...
		comboBox.addItem("CFPGrowth_association_rules");
		comboBox.addItem("CFPGrowth_association_rules_with_lift");
	...
	private void updateUserInterfaceAfterAlgorithmSelection(String algorithmName, boolean isSelected) {
		...
		if (isSelected) {
			...
			if ("SPAM".equals(algorithmName) || "PrefixSpan".equals(algorithmName)
					|| "CM-SPAM".equals(algorithmName) || "VMSP".equals(algorithmName)) {
				...
				setParam(textFieldParam4, "Min whole time interval:", labelParam4,
						"(e.g. 0 itemsets)");
				setParam(textFieldParam5, "Max whole time interval:", labelParam5,
						"(e.g. 2 itemsets)");
				...
				// // show the parameters of this algorithm
				// hideAllParams();
				// setParam(textFieldParam1, "Choose minsup (%):", labelParam1,
				// "(e.g. 0.5 or 50%)");
				// setParam(textFieldParam2, "Min time interval:", labelParam2,
				// "(e.g. 0 itemsets)");
				// setParam(textFieldParam3, "Max time interval:", labelParam3,
				// "(e.g. 2 itemsets)");
				// setParam(textFieldParam4, "Min whole time interval:",
				// labelParam4, "(e.g. 0 itemsets)");
				// setParam(textFieldParam5, "Max whole time interval:",
				// labelParam5, "(e.g. 2 itemsets)");
				// setParam(textFieldParam6, "Max whole time interval:",
				// labelParam6, "(e.g. 2 )");
				// setParam(textFieldParam7, "Max whole time interval:", labelP,
				// "(e.g. 2 )");
				//
				// }
				// comboBox.addItem("Fournier08-Closed+time");
				// comboBox.addItem("Fournier08-Closed+time+valued_items");
					...
					|| "BIDE+".equals(algorithmName) || "BIDE+_with_strings".equals(algorithmName)
					|| "PrefixSpan_AGP".equals(algorithmName)
					|| "PrefixSpan_PostProcessingClosed".equals(algorithmName)
					|| "GSP".equals(algorithmName) || "SPADE".equals(algorithmName)
					|| "CM-SPADE".equals(algorithmName) || "CM-ClaSP".equals(algorithmName)
					|| "SPADE_Parallelized".equals(algorithmName)
					|| "SPAM_AGP".equals(algorithmName)
					|| "SPAM_PostProcessingClosed".equals(algorithmName)
					|| "ClaSP".equals(algorithmName) || "CloSpan".equals(algorithmName)
					|| "MaxSP".equals(algorithmName) || "FPGrowth_itemsets".equals(algorithmName)
					...
					|| "AprioriClose".equals(algorithmName) || "AprioriRare".equals(algorithmName)
					|| "Eclat".equals(algorithmName) || "Charm".equals(algorithmName)
					|| "Charm_MFI".equals(algorithmName) || "Charm_bitset".equals(algorithmName)
					|| "Relim".equals(algorithmName) || "Eclat_bitset".equals(algorithmName)
					|| "Pascal".equals(algorithmName) || "Zart".equals(algorithmName)) {
				// show the parameters of this algorithm
				hideAllParams();
				...
				// show the parameters of this algorithm
				hideAllParams();
				setParam(textFieldParam1, "Choose threshold (%):", labelParam1,
						"(e.g. 0.15 or 15%)");
			}
				...
				// show the parameters of this algorithm
				hideAllParams();
				setParam(textFieldParam1, "Choose expected support (%):", labelParam1,
						"(e.g. 0.10)");
			}
			else if ("FPGrowth_association_rules".equals(algorithmName)
					|| "Apriori_association_rules".equals(algorithmName)
					|| "RuleGrowth".equals(algorithmName) || "CMRules".equals(algorithmName)
					|| "CMDeo".equals(algorithmName) || "IGB".equals(algorithmName)
			...
			else if ("CFPGrowth_association_rules_with_lift".equals(algorithmName)) {
				// show the parameters of this algorithm
				hideAllParams();
			...
			else if ("CFPGrowth_association_rules".equals(algorithmName)) {
				// show the parameters of this algorithm
				hideAllParams();
				setParam(textFieldParam1, "MIS file name:", labelParam1, "(e.g. MIS.txt)");
				setParam(textFieldParam2, "Choose minconf (%):", labelParam2, "(e.g. 0.6 or 60%)");
				...
				setParam(textFieldParam4, "Choose itemset count per sequence:", labelParam4,
						"(e.g. 7)");
				...
				setParam(textFieldParam2, "Choose max. distinct items:", labelParam2, "(e.g. 1000)");
				setParam(textFieldParam3, "Max. item count per transaction:", labelParam3,
						"(e.g. 10)");
				lblChooseInputFile.setVisible(false);
				buttonInput.setVisible(false);
				textFieldInput.setVisible(false);
				...
				if (isVisible() == false) {
					System.out
							.println("There is no algorithm with this name. "
	 ...
	 */
	private static boolean runAlgorithm(String algorithmName, String inputFile, String outputFile,
			String[] parameters) {
		...
		try {
			...
			// This map is to store the mapping from ItemID to Attribute value
			// for the conversion
			// from ARFF to SPMF.
			...
			if (inputFile != null && (inputFile.endsWith(".arff") || inputFile.endsWith(".ARFF"))) {
				...
				mapItemAttributeValue = converter.convertARFFandReturnMap(originalInputFile,
						inputFile, Integer.MAX_VALUE);
			...
			if ("PrefixSpan".equals(algorithmName)) {
				...
				int minsup = (int) (getParamAsDouble(parameters[0]) * sequenceDatabase.size()); // we
																								// use
																								// a
																								// minimum
																								// support
				...
				double minsup = getParamAsDouble(parameters[0]); // we use a
																	// minimum
																	// support
																	// of 2
																	// sequences.
				...
				// itemsets.
				// This options is offered because on some database,
				// AprioriClose does not
				...
				AlgoFournierViger08 algo = new AlgoFournierViger08(minsup, minInterval,
						maxInterval, minWholeInterval, maxWholeInterval, null, false, false);
				...
				AlgoFournierViger08 algoPrefixSpanHirateClustering = new AlgoFournierViger08(
						minsup, minInterval, maxInterval, minWholeInterval, maxWholeInterval, null,
						false, false);
				algoSeqDim2.runAlgorithm(contextMDDatabase, algoPrefixSpanHirateClustering,
						algoDim, false, outputFile);
				...
				algo.runAlgorithm(inputFile, outputFile, getParamAsDouble(parameters[0])); // minsup
																							// =
																							// 106
																							// k
																							// =
				...
				algoIGB.runAlgorithm(results, database.getTransactions().size(), minconf,
						outputFile);
			...
			// algo.printStats();
				...
				// STEP 1: Applying the FHSAR algorithm to hide association
				// rules
				...
				highUtilityItemsets
						.saveResultsToFile(outputFile, database.getTransactions().size());
				...
				System.out.println("Sequence database converted.  Time spent for conversion = "
				...
				long endTIme = System.currentTimeMillis();
				System.out.println("Transaction database converted.  Time spent for conversion = "
						+ (endTIme - startTime) + " ms.");
			}
			else if ("Generate_a_sequence_database".equals(algorithmName)) {
	...
	public static void processCommandLineArguments(String[] args) {
		...
		// java -Xmx1024m -jar spmfGUIv090b.jar run PrefixSpan
		// contextPrefixSpan.txt test3.txt 60%
	 ...
	 * 
	 * @param choice
	 *            the algorithm name (e.g. "PrefixSpan")
	 */
	private void openHelpWebPageForAlgorithm(String choice) {
		...
		else if ("FPGrowth_itemsets".equals(choice)
				|| "FPGrowth_itemsets_with_strings".equals("choice")) {
		...
		else if ("CFPGrowth_association_rules".equals(choice)) {
			openWebPage("http://www.philippe-fournier-viger.com/spmf/index.php?link=documentation.php#cfpgrowth_ar");
		}
		else if ("CFPGrowth_association_rules_with_lift".equals(choice)) {
			openWebPage("http://www.philippe-fournier-viger.com/spmf/index.php?link=documentation.php#cfpgrowth_ar");
		}
	...
	/**
	 * This method ask the user to choose the input file. This method is called
	 * when the user click on the button to choose the input file.
	 */
	private void askUserToChooseInputFile() {
		try {
			...
			JOptionPane.showMessageDialog(
					null,
					"An error occured while opening the input file dialog. ERROR MESSAGE = "
							+ e.toString(), "Error", JOptionPane.ERROR_MESSAGE);
		}
	}
	...
	private void askUserToChooseOutputFile() {
		try {
			...
			JOptionPane.showMessageDialog(
					null,
					"An error occured while opening the output file dialog. ERROR MESSAGE = "
							+ e.toString(), "Error", JOptionPane.ERROR_MESSAGE);
		}
	}

	/**
	 * This method is called when the user click the "Run" button of the user
	 * interface, to launch the chosen algorithm and thereafter catch exception
	 * if one occurs.
	 */
	private void processRunAlgorithmCommandFromGUI() {
		...
		boolean succeed = runAlgorithm(choice, inputFile, outputFile, parameters); // END
																					// IF
																					// -
																					// CHECBOX
		...
		if (succeed && checkboxOpenOutput.isSelected() && lblSetOutputFile.isVisible()) {
			...
			if (desktop.isSupported(Desktop.Action.OPEN)) {
				try {
					...
					JOptionPane.showMessageDialog(
							null,
							"An error occured while opening the output file. ERROR MESSAGE = "
									+ e.toString(), "Error", JOptionPane.ERROR_MESSAGE);
				}
			}
		}
