...
public class ResultConverter {
	 ...
	 * 
	 * @param mapItemIDtoStringValue
	 *            a mapping between item ID (key) and attribute value (value).
	 * @param outputFile
	 *            the path of an output file to be converted
	 * @param outputFileConverted
	 *            the path of the result file to be written to disk
	 * @throws IOException
	 *             an exception is thrown if there is an error reading/writing
	 *             files
	 */
	public void convert(Map<Integer, String> mapItemIDtoStringValue, String outputFile,
			String outputFileConverted) throws IOException {
		...
		while ((thisLine = myInputResult.readLine()) != null) {
			...
			if (thisLine.isEmpty() == false) {
				...
				for (int i = 0; i < split.length; i++) {
					...
					// if the character "#" is met, it means that the rest of
					// the line
					// does not contains items, we note it and we write the
					// token directly
					...
					if (token.startsWith("#") || noItemsLeft) {
						...
						if (itemID == null) {
							...
							// convert the item to its string and write it to
							// the output file
	 ...
	 * 
	 * @param inputDB
	 *            an input file providing the mapping between item ID (key) and
	 *            attribute value (value) as metadata.
	 * @param outputFile
	 *            the path of an output file to be converted
	 * @param outputFileConverted
	 *            the path of the result file to be written to disk
	 ...
	 *             files
	 */
	public void convert(String inputDB, String outputFile, String outputFileConverted)
			throws IOException {
	...
	/**
	 * Get the integer representation of a string or null if the string is not
	 * an integer.
	 * 
	 * @param string
	 *            a string
