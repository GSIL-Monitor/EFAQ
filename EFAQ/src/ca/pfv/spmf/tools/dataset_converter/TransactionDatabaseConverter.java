...
public class TransactionDatabaseConverter {
	...
	String	output;			// the path of the file to be written to disk in
								// SPMF format
	int		lineCount	= 0;	// the number of sequences in the input file

	/**
	 * This method converts a transaction database from a given format to the
	 * SPMF format.
	 * 
	 * @param input
	 *            the path of the input file
	 * @param output
	 *            the path of the file to be written to disk in SPMF format
	 * @param inputFileformat
	 *            the format of the input file
	 * @param lineCount
	 *            the number of lines from the input file that should be
	 *            converted
	 * @throws IOException
	 *             an exception is thrown if there is an error reading/writing
	 *             files Otherwise, null.
	 */
	public void convert(String input, String output, Formats inputFileformat, int lineCount)
			throws IOException {
	...
	/**
	 * This method convert a transaction database in ARFF format to SPMF format
	 * and return a map of key = item id value = corresponding attribute value.
	 * This method is to be used by the GUI version of SPMF that need to keep
	 * the mapping between item IDs and attribute value in memory to avoid an
	 * extra database scan.
	 * 
	 * @param inputFile
	 *            the path of the file to be converted
	 * @param outputFile
	 *            the path for saving the converted file
	 * @param lineCount
	 *            the number of lines of the input file to be converted
	 * @return a map of entry (key : itemID, value: attribute-value) if the
	 *         input format is ARFF.
	 * @throws IOException
	 ...
	 * @throws IOException
	 *             exception if error while reading/writing files.
	 * @return a map where an entry indicates for an item (key), the
	 *         corresponding attribute value (value).
	 */
	private Map<Integer, String> convertARFF(boolean ignoreMissingValues,
			boolean returnMapItemIDValue) throws IOException {
		// This map will be used to store mapping from item id (key) to
		// attribute value (value).
		// It is used only if returnMapItemIDValue is set to true. This is used
		// by the GUI of SPMF
		// which need to keep this information in memory to avoid an extra
		// database scan after an algorithm
		// is applied.
		...
		try {
			...
			while ((thisLine = myInput.readLine()) != null) {
				...
				if (thisLine.startsWith("@ATTRIBUTE") || thisLine.startsWith("@attribute")) {
					...
					// get the first position of the attribute name after the
					// space before it
					...
					// If quotes are use
					if (useQuotes) {
						// get the position of the character just before the
						// second quote
						...
						// get the position of the character just before the
						// space after the attribute name
				...
				// For example:
				// {2 W, 4 "class B"}
				// where each instance is a pair indicating the attribute number
				// and the value.
				...
				if (thisLine.startsWith("{")) {
					...
					// we will use a HashSet<Integer> to remember which
					// attribute
					// position are included and which one are not.
					// This is important because if an attribute is ommited,
					// the value 0 should be used according to the ARFF
					// specification.
					...
					for (String entry : thisLine.split(",")) {
						...
						if (itemID == null) {
							// if it is the first time that we see this
							// attribute,
							// increase item ID.
							...
							unseenAttributeValues.append("@ITEM=" + itemID + "="
									+ listAttributeNames.get(i) + "=" + val + "\n");
						}
					...
					// We will put the value 0 for all position that have not
					// been
					// seen.
					for (int i = 0; i < attributeCount; i++) {
						...
						if (positionProcessed.contains(i) == false) {
							...
							// get the corresponding item id
							Map<String, Integer> mapValueToItemID = mapAttributeValuesItemsID
									.get(i);
							...
							if (itemID == null) {
								...
								// increase item ID.
								itemID = ++lastItemAdded;
								// record the itemID that is given for this
								// value
								mapValueToItemID.put(val, itemID);
								if (mapItemsIDsToAttributeValues != null) {
									mapItemsIDsToAttributeValues.put(itemID,
											listAttributeNames.get(i) + "=" + val);
								}
								// add the unseen attribute value to the string
								// for
								// unseen attribute values.
								unseenAttributeValues.append("@ITEM=" + itemID + "="
										+ listAttributeNames.get(i) + "=" + val + "\n");
