...
public class SequenceDatabaseConverter {
	...
	String			output;			// the path of the file to be written to
										// disk in SPMF format
	int				lineCount	= 0;	// the number of sequences in the input
										// file
		...
		try {
			...
			while ((thisLine = myInput.readLine()) != null) {
				...
				if (thisLine.length() >= 11) {
					// for each integer on this line, we consider that it is an
					// item
				...
				count++; // we increase the number of line that was read until
							// now
	...
	private void convertBMS() {
		...
		try {
			...
			while ((thisLine = myInput.readLine()) != null) {
				...
				int id = Integer.parseInt(split[0]); // id of the user on this
														// line
				int val = Integer.parseInt(split[1]); // webpage viewed by this
														// user

				// if the id of the current user is not the same as the previous
				// line
				if (lastId != id) {
					...
					lastId = id; // remember the current user id for this line
									// so that we know it for next line
				}
	...
	private void convertKosarak() throws IOException {
		...
		try {
			...
			while ((thisLine = myInput.readLine()) != null) {
				...
				for (String value : split) {
					// we convert to integer and write it to file (it is an
					// item)
					Integer item = Integer.parseInt(value);
					writer.write(item + " -1 "); // write an itemset separator
				}
	...
	private void convertIBMGenerator() {
		...
		try {
			...
			// Variable to remember if we have written -1 after a group of items
			// or not
			// (because in the binary format, at the end of a line there is no
			// -1 before the -2
			...
			while (myInput.available() != 0) {
				...
				// else it is an item
					...
					lastMinus1 = false; // to remember that we need to write a
										// -1
				}
			}
			myInput.close();
