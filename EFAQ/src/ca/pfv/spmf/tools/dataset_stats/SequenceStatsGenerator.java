...
public class SequenceStatsGenerator {
	...
	public void getStats(String path) throws IOException {
		...
		List<Sequence> sequences = new ArrayList<Sequence>(); // A sequence
																// database is
																// stored as a
																// list of
																// sequences
		...
		try {
			...
			while ((thisLine = myInput.readLine()) != null) {
				...
				// we create a new sequence object to store the sequence that
				// correspond to this line.
				Sequence sequence = new Sequence(i++);
				// we create a list of integer to store the current itemset from
				// the sequence
				// that correspond to this line.
				List<Integer> itemset = new ArrayList<Integer>();
				// For each token
				for (String token : tokens) {
					// if the token starts with "<" it means that it is a
					// timestamp
					if (token.codePointAt(0) == '<') {
						// we just ignore it for statistics..
					}
					// if the token is "-1" it means that it is the end of an
					// itemset
					else if (token.equals("-1")) {
						// we add the itemset to the sequence
						sequence.addItemset(itemset);
						// we reset the variable itemset to read the next
						// itemset
						itemset = new ArrayList<Integer>();
					}
					// if the token is "-2", it indicates the end of this
					// sequence and the
					// end of the line
		...
		// we initialize some variables that we will use to generate the
		// statistics
		java.util.Set<Integer> items = new java.util.HashSet<Integer>(); // the
																			// set
																			// of
																			// all
																			// items
		List<Integer> sizes = new ArrayList<Integer>(); // the lengths of each
														// sequence
		List<Integer> itemsetsizes = new ArrayList<Integer>(); // the lengths of
																// each itemset
		List<Integer> differentitems = new ArrayList<Integer>(); // the number
																	// of
																	// different
																	// item for
																	// each
																	// sequence
		List<Integer> appearXtimesbySequence = new ArrayList<Integer>(); // the
																			// average
																			// number
																			// of
																			// times
		...
		for (Sequence sequence : sequences) {
			...
			// the key is an item
			// the value is the number of occurences of the item until now for
			// this sequence
			...
			// we put the number of times in a global list
		...
		System.out.println("Average number of itemsets per sequence : " + calculateMean(sizes)
				+ " standard deviation: " + calculateStdDeviation(sizes) + " variance: "
		...
		System.out.println("Average number of items per itemset : " + calculateMean(itemsetsizes)
				+ " standard deviation: " + calculateStdDeviation(itemsetsizes) + " variance: "
