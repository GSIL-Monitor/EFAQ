...
public class AlgoAprioriHT {
	...
	// For statistics
	protected int		totalCandidateCount	= 0;	// total number of
	...
	private int			hash_tree_branch_count;	// the number of branches in
													// the hash tree
	 ...
	 * @param hash_tree_branch_count
	 *            the number of child nodes for each node in the hash tree
	 * @throws IOException
	...
	public void runAlgorithm(double minsup, String input, String output, int hash_tree_branch_count)
		...
		do {
			...
			// This is done efficiently because the candidates are stored in a
			// hash-tree.
	...
	private ItemsetHashTree generateCandidateSizeK(ItemsetHashTree candidatesK_1, int k) {
		...
		// To generate an itemsets, we need to use two itemsets from the same
		// leaf node.
	...
	/**
	 * Method to generate candidates of size k from two list of itemsets of size
	 * k-1
	 * 
	 * @param list1
	 *            the first list
	 * @param list2
	 *            the second list (may be equal to the first list)
	 * @param candidatesK
	 *            the hash-tree containing the candidates of size k-1
	 * @param newCandidates
	 *            the hash-tree to store the candidates of size k
	 */
	private void generate(List<Itemset> list1, List<Itemset> list2, ItemsetHashTree candidatesK_1,
		...
		for (int i = 0; i < list1.size(); i++) {
			...
			// if the two lists are the same, we will start from i+1 in the
			// second list
			...
			for (; j < list2.size(); j++) {
				...
				// a new candidate.
				// There is two cases depending if the last item of itemset1 is
				// smaller
				// or greater than the last item of itemset2. We do this just to
				// make
				// sure that we add items in the new candidate according to the
	 ...
	 * 
	 * @param frequent1
	 *            The frequent itemsets of size 1
	 ...
	 * @param itemset
	 *            the itemset
	 * @param hashtreeCandidatesK_1
	 *            the hash-tree of the previous level
	 * @return
	 */
	protected boolean allSubsetsOfSizeK_1AreFrequent(int[] itemset,
			ItemsetHashTree hashtreeCandidatesK_1) {
		...
		for (int posRemoved = 0; posRemoved < itemset.length; posRemoved++) {
			...
			if (hashtreeCandidatesK_1.isInTheTree(itemset, posRemoved) == false) { // if
																					// we
																					// did
																					// not
																					// find
	 ...
	 * 
	 * @param item
	 *            the item contained in the itemset.
	 * @param support
	 *            the support of the item.
	 * @throws IOException
	 *             if an error happens while writing to file.
	 */
