...
public class ItemsetHashTree {
	 ...
	 * 
	 * @param itemsetSize
	 *            the size of the itemsets that will be inserted in the tree
	 */
	...
	public void insertCandidateItemset(Itemset itemset) {
		candidateCount++; // increase the counter for the number of itemsets in
							// the tree
	 ...
	 *            the itemset to be inserted
	 * @param level
	 *            the current level in the tree (root = level 1 ...)
	 */
	...
	class InnerNode extends Node {
		...
		Node	childs[]	= new Node[branch_count];	// contains a list of
														// child nodes
	}
	...
	/**
	 * Recursive method for increasing the support count of all itemsets
	 * contained in the hash-tree that are contained in a transaction.
	 * 
	 * @param transaction
	 *            the transaction
	 * @param node
	 *            the current node that is explored
	 * @param firstPositionToCheck
	 *            the current position in the transaction to be explored
	 * @param prefix
	 *            the current prefix that is being explored
	 */
	private void updateSupportCount(int[] transaction, InnerNode node, int firstPositionToCheck,
			int[] prefix) {
		// the index of the last item in the transaction
		int lastPosition = transaction.length - 1;
		// the index of the last item that can be the first item in lexical
		// order of an itemset in the transaction
		...
		for (int i = firstPositionToCheck; i <= lastPositionToCheck; i++) {
			...
			if (nextNode == null) {
				// there is no node, so nothing to do! we stop exploring this
				// path...
			}
	 ...
	 *            the itemset
	 * @param posRemoved
	 *            the position of an item that should be ignored in this itemset
	...
	public boolean isInTheTree(int[] itemset, int posRemoved) {
		...
		for (int i = 0; i < itemset.length; i++) {
			...
			if (count == itemsetSize) {
				// if the leaf node is null, the itemset is not there, so we
				// return false.
				if (node == null) {
					return false;
				}
				...
				while (first <= last) {
					...
					if (sameAs(list.get(middle), itemset, posRemoved) < 0) {
						first = middle + 1; // the itemset compared is larger
											// than the subset according to the
											// lexical order
					}
					else if (sameAs(list.get(middle), itemset, posRemoved) > 0) {
						last = middle - 1; // the itemset compared is smaller
											// than the subset is smaller
	 ...
	 *            another itemset
	 * @param posRemoved
	 *            the position of an item that should be ignored in the second
	 *            itemset
	 * @return
	 */
	...
	/**
	 * A method that check if an itemset is equal to another itemset called
	 * "prefix" + itemI + itemJ
	 * 
	 * @param itemset1
	 *            the itemset
	 * @param prefix
	 *            the itemset called "prefix"
	 * @param itemI
	 *            an item that should be appended to prefix
	 * @param itemJ
	 *            a second item that should be appended to prefix
	 * @return
	 */
	...
	// level.add(candidate);
	// }else{
	// int insertPos = Collections.binarySearch(level, candidate, new
	// Comparator<Itemset>(){
	// @Override
