...
public class AlgoPASCAL {
	...
	private List<ItemsetPascal> generateCandidate2(List<ItemsetPascal> frequent1) {
		...
		for (int i = 0; i < frequent1.size(); i++) {
			...
			for (int j = i + 1; j < frequent1.size(); j++) {
				...
				itemset.pred_sup = Math.min(itemset1.getAbsoluteSupport(),
						itemset2.getAbsoluteSupport());
		...
		for (int i = 0; i < levelK_1.size(); i++) {
			...
			for (int j = i + 1; j < levelK_1.size(); j++) {
				...
				if (allSubsetsOfSizeK_1AreFrequent(newItemsetPascal, levelK_1)) {
					// ------ CODE SPECIFIC TO PASCAL --------
					// if the candidate is not a generator then the support of
					// this
					// itemset is the same as the lowest support of its subsets
					// of size k-1
			...
			while (first <= last) {
				...
				if (sameAs(levelK_1.get(middle), candidate, posRemoved) < 0) {
					...
					if (isAGenerator == false) {
						// IF NOT THEN THE candidate itemset is also not a
						// generator
	...
	void saveItemsetToFile(ItemsetPascal itemset) throws IOException {
		writer.write(itemset.toString() + " #SUP: " + itemset.getAbsoluteSupport()
				+ " #IS_GENERATOR " + itemset.isGenerator);
		writer.newLine();
		itemsetCount++;
	}
