 ...
 * DCI_Closed algorithm finds all closed itemsets in a transaction database. <br/>
 * <br/>
 * 
...
public class AlgoDCI_Closed_Optimized {
	 ...
	 * 
	 * @param input
	 *            the path of an input file (transaction database).
	 * @param output
	 *            the path of the output file for writing the result
	 * @param minsup
	 *            a minimum support threshold
	 * @throws IOException
	 *             exception if error while writing/reading files
	 */
	public void runAlgorithm(String input, String output, int minsup) throws IOException {
		...
		// (0) SCAN TO KNOW THE DATABASE SIZE AND # OF ITEMS TO INITIALISE
		// BIT-MATRIX
		firstScan(input);
		...
		// (2) INITIAL VARIABLES FOR THE FIRST CALL TO THE "DCI_CLOSED"
		// PROCEDURE
		...
		System.out.println(" Total time ~: " + (System.currentTimeMillis() - startTimestamp)
				+ " ms");
		// close the file
		writer.close();
	}
	...
	/**
	 * Scan database to know the database size and number of items to initialize
	 * the bit matrix.
	 * 
	 * @param input
	 *            the input file
	 ...
	 * 
	 * @param firstime
	 *            true if this method is called for the first time
	 * @param closedset
	 *            the closed set (see paper).
	 * @param bitset
	 *            the tids set of the closed set
	 * @param postset
	 *            the postset (see paper for full details)
	 * @param preset
	 *            the preset (see paper)
	 * @param matrix
	 *            the modified matrix
	 * @param originalMatrix
	 *            the original bitmatrix
	 * @exception IOException
	 *                if error writing the output file
	 */
	private void dci_closed(boolean firstTime, List<Integer> closedset, BitSet bitset,
			List<Integer> postset, List<Integer> preset, BitMatrix matrix, BitMatrix originalMatrix)
			throws IOException {
			...
			if (newgenTIDs.cardinality() >= minSuppRelative) {
				...
				if (is_dup(newgenTIDs, preset, matrix) == false) {
					...
					for (Integer j : postset) {
						// if i is smaller than j according to the total order
						// on items
						if (smallerAccordingToTotalOrder(i, j, originalMatrix)) {
							...
							if (isAllContainedIn(newgenTIDs, matrix.getBitSetOf(j))) {
								closedsetNew.add(j);
								// recalculate TIDS of closedsetNEW by
								// intersection
					...
					// L16: recursive call
					// FIXED: we have to make a copy of preset before the
					// recursive call
					List<Integer> presetNew = new ArrayList<Integer>(preset);
					if (firstTime) {
						// THIS IS THE "Dataset projection" optimization
						// described in the TKDE paper.
						...
						dci_closed(false, closedsetNew, replacement, postsetNew, presetNew,
								projectedMatrix, matrix);
					}
					else {
	...
	/**
	 * Check if an item is smaller than another according to the support
	 * ascending order or if the support is the same, use the lexicographical
	 * order.
	 */
	 ...
	 * 
	 * @param newgenTIDs
	 *            the tidset of newgen
	 * @param preset
	 *            the itemset "preset"
	 * @param matrix
	 *            the current transaction database as a bit matrix
	 */
	private boolean is_dup(BitSet newgenTIDs, List<Integer> preset, BitMatrix matrix) {
		...
		for (Integer j : preset) {
			...
			if (isAllContainedIn(newgenTIDs, matrix.getBitSetOf(j))) {
				return true; // FIXED: IN ORIGINAL PAPER THEY WROTE FALSE, BUT
								// IT SHOULD BE TRUE
			}
		}
		return false; // FIXED: IN ORIGINAL PAPER THEY WROTE TRUE, BUT IT SHOULD
						// BE FALSE
	}

	/**
	 * Project the bitmatrix with a given bitset. This removes all the columns
	 * in the bitmatrix that do not contain a 1 in the given bitset.
	 * 
	 * @param matrix
	 *            the original bitmatrix
	 * @param bitset
	 *            a bitset
	 * @param projectedsize
	 *            the number of transaction in the projected bitmatrix.
	 ...
	 *            an input file path.
	 * @throws IOException
	 *             exception if an error while reading the file
	 */
