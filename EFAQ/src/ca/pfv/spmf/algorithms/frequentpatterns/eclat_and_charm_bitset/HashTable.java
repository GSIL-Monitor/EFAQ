...
/**
 * This class represents a hash table as used by the bitset version of the Charm
 * algorithm.
 * 
...
class HashTable {
	 ...
	 * 
	 * @param size
	 *            size of the internal array for the hash table.
	 */
	...
	public boolean containsSupersetOf(Itemset itemsetObject) {
		...
		for (Object object : table[hashcode]) {
			...
			if (itemsetObject2.itemset.size() == itemsetObject.itemset.size()
					&& itemsetObject2.itemset.containsAll(itemsetObject.itemset)) { // FIXED
																					// BUG
																					// 2010-10:
																					// containsAll
																					// instead
	...
	/**
	 * Calculate the hashcode of an itemset as the sum of the tids of its tids
	 * set, modulo the internal array length.
	 * 
	 * @param itemsetObject
	 *            an itemset.
	...
	public int hashCode(Itemset itemsetObject) {
		int hashcode = 0;
		// for (int bit = bitset.nextSetBit(0); bit >= 0; bit =
		// bitset.nextSetBit(bit+1)) {
		// for each tid in the tidset
		for (int tid = itemsetObject.tidset.nextSetBit(0); tid >= 0; tid = itemsetObject.tidset
				.nextSetBit(tid + 1)) {
			// make the sum
			hashcode += tid;
		}
