...
/**
 * This is a bitset-based implementation of the CHARM algorithm that was
 * proposed by MOHAMED Zaki et al (2002).The paper describing CHARM: <br/>
 * <br/>
 * 
 ...
 * <br/>
 * 
 * This version implement TIDs sets as bit vectors. Note however that Zaki have
 * proposed other optimizations (e.g. diffset), not used here.
 * 
 * @see Itemset
 * @see HashTable
 * @see ITSearchTree
...
public class AlgoCharm_Bitset_saveToFile {
	...
	int						tidcount;										// the
																			...
																			// of
																			// transactions
	BufferedWriter			writer		= null;							// object
																			// to
																			// write
																			// the
																			// output
	 ...
	 * 
	 * @param input
	 *            an input file path of a transation database
	 * @param output
	 *            an output file path for writing the result or if null the
	 *            result is saved into memory and returned
	 * @param minsup
	 *            the minimum suppport
	 * @param hashTableSize
	 *            the hashtable size for the internal array
	 * @throws IOException
	 *             exception if error while writing the file.
	 */
	public void runAlgorithm(String input, String output, double minsup, int hashTableSize)
			throws IOException {
		// create hash table
		...
		while (((line = reader.readLine()) != null)) {
			...
			for (String stringItem : lineSplited) {
				...
				// add the tid of the current transaction to the tidset of the
				// item
				if (tids == null) {
					tids = new BitSet();
		...
		for (List<Itemset> hashE : hash.table) {
			...
			if (hashE != null) {
				...
				for (Itemset itemsetObject : hashE) {
					// write the itemset
					writer.write(itemsetObject.toString() + " #SUP: " + itemsetObject.cardinality);
					writer.newLine();
					// increase the count
					itemsetCount++;
				}
	 ...
	 * @param node2
	 *            the second tidset
	 * @return true if it the first tidset contains the second one, otherwise
	 *         false.
	 */
