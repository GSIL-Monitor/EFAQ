...
/**
 * This is an implementation of the CFPGrowth algorithm. CFPGrowth was proposed
 * in this paper: <br/>
 * <br/>
 * 
...
public class AlgoCFPGrowth {
	...
	// for statistics
	private long				startTimestamp;			// start time of the
															// latest execution
	private long				endTime;					// end time of the
															// latest execution
	private int					transactionCount	= 0;	// transaction count
															// in the database
	private int					itemsetCount;				// number of freq.
															// itemsets found
	 ...
	 * @param MISIn
	 *            path to a file containing the MIS thresholds.
	...
	public Itemsets runAlgorithm(String input, String output, String MISIn)
		...
		while (((line = reader.readLine()) != null)) {
			...
			// split the current transaction into items (they are separated by
			// spaces)
	 ...
	 * 
	 * @param input
	 *            path to the file containing the MIS values
	 * @throws IOException
	 *             if error occurs while reading the file or the file does not
	 *             exist
	 */
	...
	private void cfpgrowth(MISTree tree, int[] prefixAlpha, int prefixSupport,
		...
		if (tree.headerList.size() == 1) {
			...
			if (node.nodeLink == null) {
				// If the support of this node is higher than the MIS of the
				// first item
	...
	private void cfpgrowthMoreThanOnePath(MISTree tree, int[] prefixAlpha, int prefixSupport,
		...
		for (int i = tree.headerList.size() - 1; i >= 0; i--) {
			...
			for (List<MISNode> prefixPath : prefixPaths) {
				...
				for (int j = 1; j < prefixPath.size(); j++) {
					...
					if (mapSupportBeta.get(node.itemID) == null) {
						...
						mapSupportBeta
								.put(node.itemID, mapSupportBeta.get(node.itemID) + pathCount);
					}
				}
			}
			...
			if (treeBeta.root.childs.size() > 0) {
				...
				// recursive call to the main method to mine the conditional
				// tree
	 ...
	 *            an itemset
	 * @param lastItem
	 *            an item that should be appended to the itemset
	 * @param support
	 *            the support of "itemset" + "item".
	 */
