...
public class estTree {
	...
	/********************************************************************
	 * Recursive method for updating the counters of itemsets that belong to a
	 * given transaction (Phase 2: count updating phase).
	 * 
	 * @param currentNode
	 *            a tree node
	 * @param transaction
	 *            the transaction for updating
	 * @param ind
	 *            depth of the branch ending at the current node
	 ********************************************************************/
	...
	double estimateCount(int[] itemset) {
		...
		double C_upper = minsig * getN(k - (n - 1)) * Math.pow(d, n - 1) + (1 - Math.pow(d, n - 1))
				/ (1 - d);
		if (min > C_upper)
			min = C_upper;
		return min;
	 ...
	 *            the transaction
	 * @param the
	 *            depth of the current node with respect to the root of the tree
	 * @param itemset
	 ********************************************************************/

	public void insert_n_itemsets(estNode currentNode, List<Integer> transaction, int ind,
			int[] itemset) {
