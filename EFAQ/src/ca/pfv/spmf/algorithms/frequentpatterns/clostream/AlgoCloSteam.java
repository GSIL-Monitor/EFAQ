...
public class AlgoCloSteam {
	...
	public void processNewTransaction(Itemset transaction) {
		...
		// Create a set to store the combined cidlist of items in the
		// transaction
		...
		for (Integer cid : cidset) {
			...
			// Check if the intersection calculated in the previous step is in
			// Temp
			boolean found = false;
		...
		for (Map.Entry<Itemset, Integer> xc : tableTemp.entrySet()) {
			...
			if (x.isEqualTo(ctc)) {
				...
				// otherwise the itemset "x" is added to the table of closed
				// itemsets
				tableClosed.add(x);
