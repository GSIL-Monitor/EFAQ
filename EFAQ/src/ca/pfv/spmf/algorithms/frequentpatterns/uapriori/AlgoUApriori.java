...
public class AlgoUApriori {
	...
	// stats
	protected int							totalCandidateCount	= 0;	// number
																		// of
																		// candidates
																		// generated
	protected int							databaseScanCount	= 0;	// number
																		// of
																		// database
																		// scan
	protected long							startTimestamp;			// start
																		// time
																		// of
																		// latest
																		// execution
	protected long							endTimestamp;				// end
																		// time
																		// of
																		// latest
																		// execution
	 ...
	 * 
	 * @param database
	 *            the database for applying this algorithm
	 */
	 ...
	 * @throws IOException
	 *             exception if error reading/writing files
	 */
	 ...
	 *            the itemset
	 * @throws IOException
	 *             exception if error writing the itemset to the file
	 */
	...
	/**
	 * Take a set of candidates and compare them with the min expected support
	 * to keep only the itemset meeting that minimum threshold.
	 * 
	 * @param minsupp
	 *            the minimum expected threshold
	 * @param candidatesK
	 *            a set of itemsets of size k
	 * @return the set of frequent itemsets of size k
	 * @throws IOException
	 *             exception if error writing output file
	 */
	protected Set<ItemsetUApriori> createLevelWithFrequentCandidates(double minsupp,
			Set<ItemsetUApriori> candidatesK) throws IOException {
			...
			for (ItemsetUApriori candidate : candidatesK) {
				...
				for (ItemUApriori item : candidate.getItems()) {
					...
					for (ItemUApriori itemT : transaction.getItems()) {
						...
						// if the lexical order is not respected then it is
						// impossible
	...
	protected Set<ItemsetUApriori> generateCandidateSizeK(Set<ItemsetUApriori> levelK_1) {
		...
		for (int i = 0; i < levelK_1.size(); i++) {
			...
			for (int j = 0; j < levelK_1.size(); j++) {
				...
				if (missing != null) {
					// Then, create a new candidate by combining itemset1 and
					// itemset2
	 ...
	 */
	protected boolean allSubsetsOfSizeK_1AreFrequent(ItemsetUApriori candidate,
			Set<ItemsetUApriori> levelK_1) {
		// To generate all the set of size K-1, we will proceed
		// by removing each item, one by one.

		// if only one item, return true because the empty set is always
		// frequent
		if (candidate.size() == 1) {
			return true;
		}
