...
public class ItemsetUApriori {
	...
	/**
	 * Checks if this itemset is lexically smaller than a given itemset of the
	 * same size.
	 * 
	 * @param itemset2
	 *            a given itemset
	 ...
	 * itemset. If all these conditions are satisfied, this method return the
	 * last item of itemset2. Otherwise it returns null.
	 * 
	...
	ItemUApriori allTheSameExceptLastItem(ItemsetUApriori itemset2) {
		...
		for (int i = 0; i < items.size(); i++) {
			...
			if (i == items.size() - 1) {
				// the one from items should be smaller (lexical order) and
				// different than the one of itemset2
