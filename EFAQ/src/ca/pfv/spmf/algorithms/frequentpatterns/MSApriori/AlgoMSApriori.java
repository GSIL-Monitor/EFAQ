...
public class AlgoMSApriori {
	...
	// the array of MIS value where the position i indicate the MIS of item with
	// ID i.
	int							MIS[];

	// for statistics
	protected long				startTimestamp;	// start time of latest
													// execution
	protected long				endTimestamp;		// end time of latest
													// execution
	private int					itemsetCount;		// number of frequent
													// itemsets generated
	 ...
	 * 
	 * @param input
	 *            an input file containing a transaction database
	 * @param output
	 *            an output file path for writing the result
	 * @param beta
	 *            the parameter Beta for generating MIS values for all items
	 *            (see paper)
	 * @param LS
	 *            the parameter LS for generating MIS values for all items (see
	 *            paper)
	 * @throws IOException
	 *             exception if error while writing the output file
	 */
	public void runAlgorithm(String input, String output, double beta, double LS)
			throws IOException {
		...
		while (((line = reader.readLine()) != null)) {
			...
			for (int i = 0; i < lineSplited.length; i++) {
				...
				if (count == null) {
					...
					// if this is the largest item ID encountered, then remember
					// it
					if (item > maxItemID) {
						maxItemID = item;
					}
		...
		for (Entry<Integer, Integer> entry : mapItemCount.entrySet()) {
			// add the item to M
			M.add(entry.getKey());
			// calculate the MIS value for that item by using the formula
			// described in the paper
			MIS[entry.getKey()] = (int) (beta * entry.getValue());
			// if the MIS value for that item is lower than LS, then set it to
			// LS
		...
		// Now, the algorithm will discover itemset of size k > 1 starting from
		// k=2
		List<Itemset> level = null;
		k = 2;
		// Generate candidates and test them for k>1 by inscreasing k at each
		// iteration
		...
		do {
			...
			if (k == 2) {
				...
				// otherwise, we use the general procedure for candidate
				// generation
			...
			for (Integer[] transaction : database) {
				...
				for (Itemset candidate : candidatesK) {
					// We will check if the candidate is contained in the
					// transaction by
					...
					for (int item : transaction) {
						...
						if (item == candidate.get(pos)) {
							...
							if (pos == candidate.size()) {
								// then increase the support count of the
								// candidate
								candidate.increaseTransactionCount();
								continue loopCand;
							}
							// because of the total order, if the item at
							// position pos is larger
							// than the current item in the transaction we can
							// stop checking
							// this candidate because it will not be contained
							// in the transaction.
						}
	 ...
	 * 
	 * @param frequent1
	 *            frequent itemsets of size 1 (single items)
	 * @param mapItemCount
	 *            a map indicating the support of each item (key: item, value:
	 *            support)
	 * @return the set of candidates of size 2
	 */
	private List<Itemset> generateCandidate2(List<Integer> frequent1,
			Map<Integer, Integer> mapItemCount) {
	...
	protected List<Itemset> generateCandidateSizeK(List<Itemset> levelK_1) {
		...
		for (int i = 0; i < levelK_1.size(); i++) {
			...
			for (int j = i + 1; j < levelK_1.size(); j++) {
				...
				// itemset1 is smaller than the last item of itemset2, we will
				// combine them to generate a
				// candidate
				...
				for (int k = 0; k < itemset1.length; k++) {
					...
					// if they are not the last items, and
						...
						// the item from itemset 1 is smaller than the one from
						// itemset2
						if (itemComparator.compare(itemset1[k], itemset2[k]) < 0) { // pfv
							continue loop2; // we continue searching
						} // the item from itemset 1 is larger than the one from
							// itemset2
						else if (itemComparator.compare(itemset1[k], itemset2[k]) > 0) { // pfv
							continue loop1; // we stop searching: because of MIS
											// order
						}
					}
				}
	 ...
	 *            the second itemset
	 * @param postRemoved
	 *            a position that should be ignored from itemset "candidate" for
	 *            the comparison
	...
	public void printStats() {
		...
		System.out.println(" Maximum memory usage : " + MemoryLogger.getInstance().getMaxMemory()
				+ " mb");
