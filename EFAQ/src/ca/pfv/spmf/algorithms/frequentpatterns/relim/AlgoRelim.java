...
public class AlgoRelim {
	...
	private int		relativeMinsupp;	// the minimum support as a relative
										// value (integer)
	 ...
	 * @param output
	 *            the file path of the desired output file
	 * @throws IOException
	...
	public void runAlgorithm(double minsupp, String input, String output) throws IOException {
		...
		while (((line = reader.readLine()) != null)) {
			...
			// Otherwise sort the transaction according to the frequency of
			// items
			...
			initialDatabase.transactions.get(indexArray).add(
					transaction.subList(1, transaction.size()));
		}
		// close the input file
		reader.close();
		...
		for (int i = 0; i < items.length; i++) {
			...
			if (database.supports[i] > 0) {
				...
				if (database.supports[i] >= relativeMinsupp) {
					// (1) add the frequent itemset to the set of frequent
					// itemsets found!
				...
				// Create the new prefix for recursion by appending the item at
				// i
	 ...
	 * @param support
	 *            the support of the itemset with the item
	 * @throws IOException
