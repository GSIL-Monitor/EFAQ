 ...
 * ZAKI. The paper describing charm: <br/>
 * <br/>
 * 
 * Zaki, M. J., & Hsiao, C. J. (2002, April). CHARM: An Efficient Algorithm for
 * Closed Itemset Mining. In SDM (Vol. 2, pp. 457-473).
 * 
 * This implementation may not be fully optimized. In particular, Zaki proposed
 * various extensions that I have not implemented (for example diffsets).<br/>
 * <br/>
 * 
 * This version saves the result to a file or keep it into memory if no output
 * path is provided by the user to the runAlgorithm() method.<br/>
 * <br/>
 * 
...
public class AlgoCharm {
	...
	// parameters
	private int					minsupRelative;					// relative
																	// minimum
																	// support
	private TransactionDatabase	database;							// the
																	// transaction
																	// database

	// for statistics
	private long				startTimestamp;					// start
																	// time of
																	// the last
																	// execution
	...
	BufferedWriter				writer	= null;					// object to
																	// write the
																	// output
																	// file
	private int					itemsetCount;						// the
																	// number of
																	// patterns
																	// found
	...
	// itemsets of size 2.
	private TriangularMatrix	matrix;							// the
																	// triangular
																	// matrix
	private boolean				useTriangularMatrixOptimization;	// if the
																	// user want
																	// to use
																	// this
																	// optimization
	 ...
	 * 
	 * @param output
	 *            the filepath for saving the result
	 * @param database
	 *            a transaction database taken as input
	 * @param minsuppAbsolute
	 *            the ABSOLUTE minimum support (double)
	 * @param hashTableSize
	 *            the size of the hashtable to be used by charm
	 ...
	 * @throws IOException
	 *             if an error occurs while writting to file.
	 */
	public Itemsets runAlgorithm(String output, TransactionDatabase database, int hashTableSize,
			double minsuppAbsolute, boolean useTriangularMatrixOptimization) throws IOException {
		...
		this.minsupRelative = (int) Math.ceil(minsuppAbsolute * database.size());
	 ...
	 * 
	 * @param output
	 *            the output file path for saving the result.
	 ...
	 * @throws IOException
	 *             if an error occurs while writing the result to file.
	 */
	public Itemsets runAlgorithmWithRelativeMinsup(String output,
			boolean useTriangularMatrixOptimization, int minsupRelative) throws IOException {
		// save the parameters
	 ...
	 * 
	 * @param output
	 *            an output file path for writing the result or if null the
	...
	private Itemsets run(String output) throws IOException {
		...
		for (Entry<Integer, Set<Integer>> entry : mapItemCount.entrySet()) {
			...
			if (entry.getValue().size() >= minsupRelative) {
				...
				// set its tidset as the tidset that we have calculated
				// previously
	 ...
	 *            the current node.
	 * @throws IOException
	 *             exception if error while writing to file.
	 */
	private void extend(ITNode currNode) throws IOException {
		...
		while (i < currNode.getParent().getChildNodes().size()) {
			...
			if (brother != currNode) {
				...
				if (currNode.getTidset().equals(brother.getTidset())) {
					// we can replace the current node itemset in the current
					// node
				...
				// brother
					...
					// the union of the itemsets of the current node and its
					// brother
		...
		while (currNode.getChildNodes().size() > 0) {
			...
			extend(child); // extend it (charm is a depth-first search
							// algorithm)
	...
	/**
	 * Replace the itemset of a current node by another itemset in a subtree
	...
	/**
	 * Generate a candidate by performing the union of the current node and a
	 * brother of that node.
	 * 
	 * @param currNode
	 *            the current node
	 * @param brother
	 *            the itemset of the brother node
	 * @return a candidate or null if the resulting candidate do not have enough
	 *         support.
	 */
	private ITNode getCandidate(ITNode currNode, ITNode brother) {
		...
		if (useTriangularMatrixOptimization && currNode.getItemset().size() == 1) {
			...
			int support = matrix.getSupportForItems((Integer) currNode.getItemset().getItems()
					.toArray()[0], (Integer) brother.getItemset().getItems().toArray()[0]);
			// if not frequent
			if (support < minsupRelative) {
				// return null because the candidate would not be a frequent
				// closed itemset
				return null;
			}
		}
		...
		Set<Integer> commonTids = currNode.peformOptimizedTidSetsIntersection(brother,
				minsupRelative);
		...
		// otherwise return null because the candidate did not have enough
		// support
		return null;
	}
