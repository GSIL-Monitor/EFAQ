...
/**
 * This is an implementation of the ECLAT algorithm as proposed by ZAKI (2000). <br/>
 * <br/>
 * 
 ...
 * various extensions that I have not implemented (e.g. diffsets). <br/>
 * <br/>
 * 
...
public class AlgoEclat {
	...
	/**
	 * This is the "extend" method as described in the paper to extend a given
	 * node.
	 * 
	 * @param currNode
	 *            the current node.
	...
	private ITNode getCandidate(ITNode currNode, ITNode brother) {
		...
		if (useTriangularMatrixOptimization && currNode.getItemset().size() == 1) {
			// Get the support of the first item of each itemset by using the
			// triangular matrix
		...
		// To perform the tidset intersection, we compare the itemset with the
		// smallest tidset
		// against the one having the larger tidset. Furthermore, we stop
		// calculating the
		// tidset intersection if too many tids are not common so that minsup
		// could not be attained.
		// THIS OPTIMIZATION IS ESPECIALLY EFFECTIVE FOR SPARSE DATASETS AND LOW
		// MINSUP VALUES
		...
		if (brotherSize < currNodeSize) {
			...
			for (Integer tid : brother.getTidset()) {
				...
				brotherSize--;
				// if it is not possible to have a frequent itemset giving the
				// number of tids left, then stop
