...
public class AlgoTRuleGrowth_withStrings {
	 ...
	 *            minsup as a a relative value (integer)
	 * @param minConfidence
	 *            minimum confidence (a value between 0 and 1).
	 * @param input
	 *            the input file path
	...
	public void runAlgorithm(String input, String output, int relativeMinSupport,
		...
		// IMPORTANT : THIS IS A FIX SO THAT THE DEFINITION IS THE SAME AS IN
		// THE ARTICLE!!
	 ...
	 */
	private void expandLeft(String[] itemsetI, String[] itemsetJ, Collection<Integer> tidsI,
			Collection<Integer> tidsIJ // ,
	// Map<Integer, Occurence> mapOccurencesJ
	) throws IOException {

		if (itemsetI.length == 2 && itemsetI[0].equals("a") && itemsetI[1].equals("b")
				&& itemsetJ[0].equals("d")) {
		...
		for (Entry<String, Set<Integer>> entry : frequentItemsC.entrySet()) {
			...
			if (tidsIC_J.size() >= minsuppRelative) {
				...
				// ---- CALCULATE ALL THE TIDS CONTAINING IC WITHIN A TIME
				// WINDOW ---
				...
				for (Integer tid : tidsI) {
					...
					for (int k = 0; k < sequence.size(); k++) {
						// For each item
						for (String item : sequence.get(k)) {
							if (contains(itemsetIC, item)) { // record the last
																// position of
																// each item in
																// IC
						...
						Iterator<Entry<String, Integer>> iter = mapAlreadySeenFromIC.entrySet()
								.iterator();
						while (iter.hasNext()) {
				...
				// Create rule and calculate its confidence: Conf(r) = sup(IUC
				// -->J) / sup(IUC)
	...
	private void addToLinked(LinkedHashMap<String, Integer> mapMostLeftFromI, String key,
			Integer value) {
		if (mapMostLeftFromI.containsKey(key)) {
			mapMostLeftFromI.remove(key);
		}
	 ...
	 * @throws IOException
	 */
	private void expandRight(String[] itemsetI, String[] itemsetJ, Set<Integer> tidsI,
	...
	// Map<Integer, Occurence> occurencesJ
		...
		for (Entry<String, Set<Integer>> entry : frequentItemsC.entrySet()) {
			...
			if (tidsI_JC.size() >= minsuppRelative) {
				...
				// ---- CALCULATE ALL THE TIDS CONTAINING JC WITHIN A TIME
				// WINDOW ---
				...
				for (Integer tid : tidsJ) {
					...
					for (int k = 0; k < sequence.size(); k++) {
						// For each item
						for (String item : sequence.get(k)) {
							if (contains(itemsetJC, item)) { // record the last
																// position of
																// each item in
																// JC
						...
						// remove items that fall outside the time window
						Iterator<Entry<String, Integer>> iter = mapAlreadySeenFromJC.entrySet()
								.iterator();
						while (iter.hasNext()) {
							Entry<String, Integer> entryMap = iter.next();
							if (entryMap.getValue() < k - windowSize + 1) {
				...
				// Create rule and calculate its confidence: Conf(r) =
				// sup(I-->JC) / sup(I)
	...
	private Map<String, Map<Integer, Occurence>> removeItemsThatAreNotFrequent(
		...
		// pass
		mapItemCount = new HashMap<String, Map<Integer, Occurence>>(); // <item,
																		// Map<tid,
																		// occurence>>
		...
		// System.out.println("NUMBER OF DIFFERENT ITEMS : " +
		// mapItemCount.size());
	 ...
	 */
	private void saveRule(Set<Integer> tidsIJ, double confIJ, String[] itemsetI, String[] itemsetJ)
			throws IOException {
		ruleCount++;
