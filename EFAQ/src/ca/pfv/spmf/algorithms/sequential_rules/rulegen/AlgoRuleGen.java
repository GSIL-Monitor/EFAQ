...
public class AlgoRuleGen {
	 ...
	 */
	public void runAlgorithm(int minsup, double minconf, String input, String output)
			throws IOException {
		...
		// STEP 1: Apply the PrefixSpan algorithm to generate frequent
		// sequential patterns
	...
	/**
	 * Try to generate a rule between two sequential patterns. The rule is
	 * generated if the pattern1 is included in pattern2 and if the confidence
	 * is high enough.
	 * 
	 * @param pattern1
	 ...
	 * @throws IOException
	 */
	private void tryToGenerateRule(SequentialPattern pattern1, SequentialPattern pattern2,
	...
	boolean strictlyContains(SequentialPattern pattern1, SequentialPattern pattern2) {
		// if pattern2 is larger or equal in size, then it cannot be contained
		// in pattern1
		...
		while (true) {
			...
			// lastly, for optimization, we check how many itemsets are left to
			// be matched.
			// if there is less itemsets left in pattern1 than in pattern2, then
			// it will
	...
	/**
	 * Print statistics to System.out about the latest execution of the
	 * algorithm.
	 */
