...
public class AlgoCMRules {
	...
	int							associationRulesCount					= 0;									// number
																												// of
																												// association
																												// rules
	int							ruleCount;																		// the
	...
	long						timeEndConvert							= 0;									// end
																												...
																												// transaction
																												// database
	long						timeEndApriori							= 0;									// end
																												// time
																												// for
																												// calculating
																												// frequent
																												// itemsets
	long						timeEndSequentialMeasures				= 0;									// end
																												// time
																												// for
																												// calculating
																												// measures
																												...
																												// sequential
																												// rules
	long						timeBeginCalculateSequentialMeasures	= 0;									// start
																												// time
																												// for
																												// calculating
																												// measures
																												...
																												// sequential
																												// rules
	long						timeEndPreprocessing					= 0;									// end
																												// time
																												// for
																												// pre-processing

	// *** parameters ***
	public int					minCSupRelative							= 0;									// min.
																												// seq.
																												// support
	public double				minSeqConfidence;																// min
																												// seq.
																												// confidence
	SequenceDatabase			sequences;																		// the
																												// sequence
																												// database
	...
	// a triangular matrix for efficiently counting the support of pairs of
	// items
	...
	/**
	 * Run the algorithm with the minsup parameter as a percentage value
	 * (double).
	 * 
	 * @param input
	...
	public void runAlgorithm(int relativeSupport, double minConfidence, String input, String output)
		...
		// This is based on the algorithm for association rule by Agrawal &
		// Srikant, 94
	...
	private Map<Integer, Set<Integer>> removeItemsThatAreNotFrequent(SequenceDatabase sequences) {
			...
			for (List<Integer> itemset : sequence.getItemsets()) {
				...
				for (int i = 0; i < itemset.size(); i++) {
					...
					if (ids == null) {
						...
						// if it is the largest item seen until now, then
						// remember that
		...
		for (Sequence sequence : sequences.getSequences()) {
			...
			while (i < sequence.getItemsets().size()) {
				...
				// if a sequence becomes empty ecause of removed items, then
				// remove it
				if (itemset.size() == 0) {
	...
	private void calculateSequentialMeasures(Rule rule, Sequence sequence) {
		...
		// right part of the rules
		Set<Integer> setAlreadySeen = new HashSet<Integer>(rule.getItemset1().size() * 3); // could
																							// be
																							// replaced
																							// with
																							// a
		...
		firstpass:
		{
			...
			for (; i < sequence.getItemsets().size(); i++) {
				...
				for (; j < itemset.size(); j++) { // FOR EACH ITEM OF THIS
													// SEQUENCE
					int item = itemset.get(j);
					// if the left part of the rule contains item J
					if (rule.getItemset1().contains(item)) { // left part of
																// rule
	...
	/**
	 * Calculate the interestingness measures of a rule and then if it is a
	 * valid rule, save it to the output file.
	 * 
	 * @param rule
	 *            an association rule that may be a valid sequential rule
	 * @throws IOException
	...
	/**
	 * Run the Agrawal algorithm for generating association rules that is
	 * modified to also find sequential rules
	 * 
	 * @param patterns
	 ...
	 * 
	 * @param lk
	 *            a itemset that is used to generate rules
	 * @throws IOException
		...
		if (k > m + 1) {
			...
			for (Itemset hm_P_1 : Hm_plus_1) {
				...
				if (conf >= minSeqConfidence) {
					...
					// if the size constraints allow further expansion of the
					// rule
	 ...
	 * "candidate" are frequent. It is similar to what is used in the Apriori
	 * algorithm for generating frequent itemsets.
	 * 
	 * @param candidate
	...
	protected boolean allSubsetsOfSizeK_1AreFrequent(Itemset candidate, Set<Itemset> levelK_1) {
		...
		// for each item of candidate, we will consider that this item is
		// removed
		for (Integer item : candidate.getItems()) {
			...
			// we will search itemsets of size k-1 to see if this itemset
			// appears
			boolean found = false;
			// for each itemset of size k-1
			for (Itemset itemset : levelK_1) {
				// if the itemset is equals to "subset", we found it and stop
				// the loop
