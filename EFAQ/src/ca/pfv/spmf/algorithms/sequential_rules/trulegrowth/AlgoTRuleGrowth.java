...
public class AlgoTRuleGrowth {
	...
	// *** for statistics ***
	long									timeStart	= 0;	// start time of
																// latest
																// execution
	long									timeEnd		= 0;	// end time of
																// latest
																// execution
	...
	// *** parameters ***
	SequenceDatabase						database;			// a sequence
																// database
	double									minconf;			// minimum
																// confidence
	 ...
	 * 
	 * @param minSupport
	 *            Minsup as a percentage (ex: 0.05 = 5 %)
	 * @param minConfidence
	 *            minimum confidence (a value between 0 and 1).
	 * @param input
	 *            the input file path
	 * @param output
	 ...
	 *            a window size
	 * @throws IOException
	 *             exception if there is an error reading/writing files
	 */
	public void runAlgorithm(double minSupport, double minConfidence, String input, String output,
	 ...
	 *            the minsup parameter as a a relative value (integer)
	 * @param minConfidence
	 *            minimum confidence (a value between 0 and 1).
	 * @param input
	 *            the input file path
	 ...
	 *             exception if there is an error reading/writing files
	 */
	public void runAlgorithm(String input, String output, int relativeMinSupport,
			double minConfidence, int windowSize) throws IOException {
		...
		for (int i = 0; i < listFrequents.size(); i++) {
			...
			for (int j = i + 1; j < listFrequents.size(); j++) {
					...
					for (Short posI : occI.occurences) {
						...
						for (Short posJ : occJ.occurences) {
							if (!posI.equals(posJ) && Math.abs(posI - posJ) <= windowSize) {
								...
								// if we have found that I is before J and J is
								// before I
				...
				if (tidsJI.size() >= minsuppRelative) {
					...
					expandRight(itemset2, itemset1, tidsJ, tidsI, tidsJI /*
																		 * ,
																		 * occurencesJ
																		 * ,
																		 * occurencesI
	 ...
	 * 
	 * @param itemsetI
	 *            the left side of a rule (see paper)
	 * @param itemestJ
	 *            the right side of a rule (see paper)
	 * @param tidsI
	 *            the tids set of I
	 * @param tidsJ
	 *            the tids set of J
	...
	// Map<Integer, Occurence> mapOccurencesJ
		...
		for (Integer tid : tidsIJ) {
			...
			// what is currently inside the window and what fall out of the
			// window.
			// We use linkedhashMap to as to keep the order of what is inserted
			// into the maps
			...
			do {
				...
				// important: if J was all there, but become smaller we need to
				// clear the
				...
				for (Integer item : sequence.get(k)) {
					// record the first position until now of each item in I or
					// J

					// if we saw J completely already, and the current item is
					// in I
				...
				if (mapMostLeftFromI.size() == itemsetI.length
						&& mapMostLeftFromJ.size() == itemsetJ.length) {

					// remove items from mostRight that fall outside the time
					// window.
					// at the same time, calculate the minimum index for items
					// of J.
					...
					// Such item c has to appear in the window before the last
					// occurence of J (before "minimum")
					// and if it was scanned before, it should not be scanned
					// again.
					...
					// for each item c after the first element of the window,
					// starting from the
					...
					for (; itemsetC >= firstElementOfWindow; itemsetC--) {
						for (Integer itemC : sequence.get(itemsetC)) {
							// if lexical order is not respected or c is
							// included in the rule already.
				...
				k--; // go to previous itemset in the sequence (we scan the
						// sequence bacward)
		...
		for (Entry<Integer, Set<Integer>> entry : frequentItemsC.entrySet()) {
			...
			if (tidsIC_J.size() >= minsuppRelative) {
				...
				// Calculate tids containing IU{c} within the time window which
				// is necessary
				...
				for (Integer tid : tidsI) {
					// get the sequence
					Sequence sequence = database.getSequences().get(tid);
					// To check if IU{c} is contained in that sequence we will
					// use a map
					...
					for (int k = 0; k < sequence.size(); k++) {
						...
						Iterator<Entry<Integer, Integer>> iter = mapAlreadySeenFromIC.entrySet()
								.iterator();
						while (iter.hasNext()) {
						...
						// if all the items of I are inside the current window,
						// then record the tid
	 ...
	 * containing the position of items at the left of an itemset. key: item
	 * value : a itemset position
	 * 
	 * @param mapMostLeftFromItemset
	 *            the map
	 * @param lastElementOfWindow
	 *            the last itemset of the window in terms of itemset position in
	 *            the sequence.
	 */
	...
	/**
	 * This method removes elements out of the current window from a hashmap
	 * containing the position of items at the right of an itemset. key: item
	 * value : a itemset position
	 * 
	 * @param mapMostLeftFromItemset
	 *            the map
	 ...
	 * @throws IOException
	 */
	private void expandRight(int[] itemsetI, int[] itemsetJ, Set<Integer> tidsI,
			Collection<Integer> tidsJ, Collection<Integer> tidsIJ // ,
	...
	// Map<Integer, Occurence> occurencesJ
		...
		for (Integer tid : tidsIJ) {
			...
			// what is currently inside the window and what fall out of the
			// window.
			// We use linkedhashMap so as to keep the order of what is inserted
			// into the maps
			// There maps are key: item value: position of an itemset
			...
			do {
				...
				// important: if I was all there, but become smaller we need to
				// clear the
				...
				if (mapMostRightFromI.size() == itemsetI.length
					...
					// remove items from mostLeft that fall outside the time
					// window.
					// at the same time, calculate the minimum index for items
					// of I.
					int minimum = 1;
					...
					// for each item c before the lst element of the window,
					// starting from the
					// first itemset just after the first occurence of I
					for (; itemsetC <= lastElementOfWindow; itemsetC++) {
						for (Integer itemC : sequence.get(itemsetC)) {
							// We will consider if we could create a rule I -->
							// J U{c}
							// If lexical order is not respected or c is
							// included in the rule already,
							...
							// if there is no tidset, create one
							if (tidsItemC == null) {
								// if we did not see "c" yet, create a new
								// tidset for "c"
								tidsItemC = new HashSet<Integer>();
								frequentItemsC.put(itemC, tidsItemC);
							}
		...
		for (Entry<Integer, Set<Integer>> entry : frequentItemsC.entrySet()) {
			...
			if (tidsI_JC.size() >= minsuppRelative) {
				...
				for (Integer tid : tidsJ) {
					...
					// To check if JU{c} is contained in that sequence we will
					// use a map
					// such that the key : item value: position of an itemset
					...
					for (int k = 0; k < sequence.size(); k++) {
						...
						for (Integer item : sequence.get(k)) {
							...
							// record the last position of the item for the map
							// of JU{C}
						...
						Iterator<Entry<Integer, Integer>> iter = mapAlreadySeenFromJC.entrySet()
								.iterator();
						while (iter.hasNext()) {
				...
				// Rule ruleI_JC = new Rule(ruleIJ.getItemset1(), itemsetJC,
				// confI_JC, tidsI_JC.size());
	 ...
	 *         value = Occurence This map allows knowing the frequency of each
	 *         item and their first and last occurence in each sequence.
	 */
	private Map<Integer, Map<Integer, Occurence>> removeItemsThatAreNotFrequent(
			SequenceDatabase database) {
		// (1) Count the support of each item in the database in one database
		// pass
		mapItemCount = new HashMap<Integer, Map<Integer, Occurence>>(); // <item,
																		// Map<tid,
																		// occurence>>
		...
		for (Sequence sequence : database.getSequences()) {
			...
			for (short j = 0; j < sequence.getItemsets().size(); j++) {
				...
				for (int i = 0; i < itemset.size(); i++) {
					...
					// if the map of occurences of that item is null, create a
					// new one
					if (occurences == null) {
					...
					// add the current itemset position to the occurences of
					// that item
					occurence.add(j);
				}
			}
