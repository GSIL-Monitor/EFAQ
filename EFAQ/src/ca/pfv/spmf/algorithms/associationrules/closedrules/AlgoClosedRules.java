...
/**
 * This is an algorithm for generating closed association rules from the set of
 * closed itemsets as proposed by Szathmary (2006). <br/>
 * <br/>
 * 
 * This algorithm is a modification of the classical algorithm for generating
 * association rules described in Agrawal & al. 1994, IBM Research Report
 * RJ9839, June 1994. It has been slightly modified for being applied to mine
 * all "closed association rules" as defined in Szathmary's thesis (2006). <br/>
 * <br/>
 * 
		...
		for (int k = 2; k < closedItemsets.getLevels().size(); k++) {
			for (Itemset lk : closedItemsets.getLevels().get(k)) {
				...
				for (Itemset hm_P_1 : H1) {
					...
					int supLkMinus_hm_P_1 = calculateSupport(itemset_Lk_minus_hm_P_1); // THIS
																						// COULD
																						// BE
																						// DONE
																						// ANOTHER
																						// WAY
																						// ?
					int supLk = calculateSupport(lk); // IT COULD PERHAPS BE
														// IMPROVED....
					double conf = ((double) supLk) / ((double) supLkMinus_hm_P_1);

					if (conf >= minconf) {
						ClosedRule rule = new ClosedRule(itemset_Lk_minus_hm_P_1, hm_P_1,
		...
		for (Itemset itemset1 : levelK_1) {
			for (Itemset itemset2 : levelK_1) {
				...
				if (missing != null) {
					...
					// System.out.println(" " + itemset1.toString() + " + " +
					// itemset2.toString() + " = " + candidate.toString());
