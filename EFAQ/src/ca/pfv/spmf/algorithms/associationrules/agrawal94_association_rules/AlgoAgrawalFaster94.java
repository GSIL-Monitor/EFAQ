...
public class AlgoAgrawalFaster94 {
	...
	private int			databaseSize	= 0;	// number of transactions in
												// database
			...
			for (Itemset lk : patterns.getLevels().get(k)) {
				...
				for (Itemset hm_P_1 : H1) {
					...
					if (usingLift) {
						...
						// of hm_P_1
						calculateSupport(hm_P_1); // if we want to calculate the
													// lift, we need to add
													// this.
						// calculate the lift
						double term1 = ((double) lk.getAbsoluteSupport()) / databaseSize;
						double term2 = ((double) itemset_Lk_minus_hm_P_1.getAbsoluteSupport())
								/ databaseSize;
					...
					// create a rule
					Rule rule = new Rule(itemset_Lk_minus_hm_P_1, hm_P_1, lk.getAbsoluteSupport(),
							conf, lift);
	...
	public void saveStatsToFile() throws IOException {
		...
		saveTextToFile(" Number of itemsets generated : " + patterns.getItemsetsCount() + "\n");
		saveTextToFile(" Number of association rules generated : " + ruleCount + "\n");
		saveTextToFile(" Total time ~ " + (endTimeStamp - startTimestamp) + " ms\n");
		saveTextToFile("===================================================\n");
	}
}
