 ...
 * "IGB : une nouvelle base g�n�rique informative des r�gles d�association" dans
...
public class AlgoIGB {
	...
	private int				databaseSize;					// number of
															// transactions in
															// the original
															// database
	...
	long					endTimeStamp	= 0;			// last execution
															// end time
	private int				ruleCount;						// the number of
															// rules found
	 ...
	 * @throws IOException
	 */
	public Rules runAlgorithm(TZTableClosed closedPatternsAndGenerators, int databaseSize,
			double minconf, String outputFilePath) throws IOException {
	...
	private void processItemset(Itemset i) throws IOException {
		...
		if (i.getRelativeSupport(databaseSize) >= minconf) { // line 3
			...
			Rule rule = new Rule(new Itemset(), i, i.getAbsoluteSupport(),
					i.getRelativeSupport(databaseSize)); // 4,5,6
			save(rule);
			return;
		}
			...
			for (Itemset i1 : closedPatternsAndGenerators.levels.get(j)) {
				...
				if (((double) i.getAbsoluteSupport() / (double) i1.getAbsoluteSupport()) >= minconf
					...
					for (Itemset genI1 : closedPatternsAndGenerators.mapGenerators.get(i1)) {
						...
						// check if there is a premise smaller than gen1 already
						// found
		...
		for (Itemset gs : lSmallestPremise) {
			...
			Rule rule = new Rule(gs, i_gs, i.getAbsoluteSupport(), (double) i.getAbsoluteSupport()
					/ (double) gs.getAbsoluteSupport());
			// We save the rule
			save(rule);
		}
	}
