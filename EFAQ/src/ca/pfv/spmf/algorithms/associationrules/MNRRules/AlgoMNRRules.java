 ...
 * Discovery and Data Mining, pages 198209, London, UK, 1998. Springer-Verlag. <br/>
 * <br/>
 * 
...
public class AlgoMNRRules {
	...
	// parameters
	private TZTableClosed	closedPatternsAndGenerators;	// closed itemsets
															// and their
															// generators
	private double			minconf;						// minimum
															// confidence
															// threshold
	private int				databaseSize;					//

	// for statistics
	long					startTimestamp	= 0;			// last execution
															// start time
	long					endTimeStamp	= 0;			// last execution
															// end time
	private int				ruleCount;						// number of rule
															// found
	 ...
	 * Run the algorithm.
	 * 
	 * @param closedPatternsAndGenerators
	 *            Closed itemsets and their associated generators.
	 * @param databaseSize
	 *            the number of transactions in the transaction database.
	 * @param minconf
	 *            minimum confidence threshold
	 * @param outputFilePath
	 *            the output file path, if the results should be saved to a
	 *            file. if null, the result are saved in memory and returned by
	 *            this method.
	 * 
	 * @return if the user chose to save to memory, this methods return the set
	 *         of IGB association rules, otherwise, the result is saved to the
	 *         output file chosen by the user.
	 * @throws IOException
	 */
	public Rules runAlgorithm(String outputFilePath, double minconf,
			TZTableClosed closedPatternsAndGenerators, int databaseSize) throws IOException {
		...
		for (Map.Entry<Itemset, List<Itemset>> entryEquivalenceClass : closedPatternsAndGenerators.mapGenerators
			...
			// if the equivalence class has no generator, then its closed
			// itemset is a generator...
			...
			for (Itemset generatorG : listGenerators) {
				// 3 - find proper supersets of G among the frequent closed
				// itemsets
				...
				for (Itemset closedItemset : supersets) {
					...
					if (conf >= minconf) {
						Rule rule = new Rule(leftSide, rightSide,
								closedItemset.getAbsoluteSupport(), conf);
						saveRule(rule);
					}
				}
			}
	...
	private void calculateSupport(Itemset itemsetToTest) { // THIS WAS CHANGED
		...
		for (List<Itemset> list : closedPatternsAndGenerators.levels) {
			if (list.size() == 0 || list.get(0).size() < itemsetToTest.size()) {
				continue; // it is not useful to consider itemsets that are
							// smaller
