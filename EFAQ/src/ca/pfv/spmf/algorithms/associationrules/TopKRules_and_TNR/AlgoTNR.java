...
public class AlgoTNR {
	...
	// the maximum number of candidates at the same time during the last
	// execution
	int					maxCandidateCount				= 0;
	int					notAdded						= 0;	// rules
																// eliminated by
																// strategy 1
	int					totalremovedCount				= 0;	// rules
																// eliminated by
																// strategy 2

	long				totalCandidatesConsideredFromR	= 0;	// the total
																// number of
																// candidates
																// processed
	long				totalRules11considered			= 0;	// the total
																// number of
																// rules with
																// only two
																// items
	...
	// Parameters
	double				minConfidence;							// minimum
																// confidence
																// threshold
	int					initialK						= 0;	// the value of
																// k set by the
																// user
	Database			database;								// the
																// transaction
																// database
	int					delta							= 0;	// the delta
																// parameter

	// Internal variables
	RedBlackTree<RuleG>	kRules;								// the top k
																// rules found
																// until now
	RedBlackTree<RuleG>	candidates;							// the
																// candidates
																// for expansion
	int					k								= 0;	// will contain
																// k + delta
	...
	BitSet[]			tableItemTids;							// [item], IDs
																// of
																// transaction
																// containing
																// the item
	// a table indicating the support of each item
	int[]				tableItemCount;						// [item],
																// support
	 ...
	 */
	public RedBlackTree<RuleG> runAlgorithm(int k, double minConfidence, Database database,
			int delta) {

		// reset statistics
		totalremovedCount = 0;
		notAdded = 0;
		MemoryLogger.getInstance().reset(); // reset utility to check memory
											// usage
		maxCandidateCount = 0;
		...
		// perform the first database scan to generate vertical database
		// representation
	...
	private void start() {
		...
		for (int itemI = 0; itemI <= database.maxItem; itemI++) {
			...
			for (int itemJ = itemI + 1; itemJ <= database.maxItem; itemJ++) {
				...
				// the cardinality() method is expensive, and we will need it
				// again later.
				...
				if (support >= minsuppRelative) {
					// generate rules I ==> J and J ==> I and remember these
					// rules
	 ...
	 *            an item I
	 * @param tidI
	 *            the set of IDs of transaction containing item I (BitSet)
	 * @param itemJ
	 *            an item J
	 * @param tidJ
	 *            the set of IDs of transaction containing item J (BitSet)
	 * @param commonTids
	 *            the set of IDs of transaction containing I and J (BitSet)
	 * @param cardinality
	 *            the cardinality of "commonTids"
	 */
	private void generateRuleSize11(Integer itemI, BitSet tidI, Integer itemJ, BitSet tidJ,
			BitSet commonTids, int cardinality) {
		// Create the rule I ==> J
	 ...
	 * 
	 * @param expandLR
	 *            if true the rule will be considered for left/right expansions
	...
	private void expandLR(RuleG ruleG) {
		// Maps to record the potential item to expand the left/right sides of
		// the rule
		// Key: item Value: bitset indicating the IDs of the transaction
		// containing the item
		...
		for (Entry<Integer, BitSet> entry : mapCountRight.entrySet()) {
			...
			if (ruleSupport >= minsuppRelative) {
				...
				// create the rule
				RuleG candidate = new RuleG(ruleG.getItemset1(), newRightItemset, ruleSupport,
						ruleG.tids1, tidsRule, ruleG.maxLeft, maxRight);
		...
		for (Entry<Integer, BitSet> entry : mapCountLeft.entrySet()) {
			...
			if (ruleSupport >= minsuppRelative) {
				...
				// create the rule
				RuleG candidate = new RuleG(newLeftItemset, ruleG.getItemset2(), ruleSupport,
						tidsLeft, tidsRule, maxLeft, ruleG.maxRight);
	...
	private void expandR(RuleG ruleG) {
		...
		for (int tid = ruleG.common.nextSetBit(0); tid >= 0; tid = ruleG.common.nextSetBit(tid + 1)) {
			...
			while (iter.hasNext()) {
				...
				// if that item is not frequent, then remove it from the
				// transaction
				if (tableItemCount[item] < minsuppRelative) {
					iter.remove();
					continue;
				...
				// If the item is smaller than the largest item in the right
				// side
	...
	private void save(RuleG rule, int support) {
		...
		RedBlackTree<RuleG>.Node lowerRuleNode = kRules.lowerNode(new RuleG(null, null,
				support + 1, null, null, 0, 0));
		...
		if (kRules.size() > k) {
			...
			if (support > this.minsuppRelative) {
				...
				RuleG lower;
				do {
					lower = kRules.lower(new RuleG(null, null, this.minsuppRelative + 1, null,
							null, 0, 0));
					if (lower == null) {
						break; // / IMPORTANT
					}
	...
	/**
	 * This method remove exceeding rules so that only k are presented to the
	 * user
	 */
