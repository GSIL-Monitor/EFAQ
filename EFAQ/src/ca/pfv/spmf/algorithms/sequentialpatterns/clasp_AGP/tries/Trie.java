...
public class Trie implements Comparable<Trie> {
	...
	/**
	 * IdList associated with the pattern to which the current trie is referring
	 * to
	 */
	...
	/**
	 * Constructor of a Trie by means of a list of NodeTrie and the IdList of
	 * the pattern associated to the trie.
	 * 
	 * @param nodes
	 *            List of nodes with which we want to initialize the Trie
	 * @param idList
	 ...
	 * 
	 * @param index
	 *            Child index in which we are interested
	 * @return the trie
	 */
	 ...
	 * @param index
	 *            Child index in which we are interested
	 * @return true if the item was removed, otherwise it means that the index
	 *         is outside the bounds of the trie
	 */
	...
	/**
	 * It returns the list of sequences Ids where the pattern referred by the
	 * Trie appears.
	 * 
	...
	/**
	 * It updates the list of sequences Ids where the pattern referred by the
	 * Trie appears
	 * 
	 * @param appearingIn
	 *            The list of sequence Ids to update
	 */
	 ...
	 *            Value of the sum of sequence identifiers to update
	 */
	...
	public List<Entry<Pattern, Trie>> preorderTraversal(Pattern p) {
		...
		if (nodes != null) {
			for (TrieNode node : nodes) {
				...
				if (child != null) {
					/*
					 * If the child is not null we make a recursive call with
					 * the new pattern
					 */
					List<Entry<Pattern, Trie>> patternsFromChild = child
							.preorderTraversal(newPattern);
	 ...
	 * 
	 * @param model
	 *            TreeModel when we want to insert the Trie nodes
	 * @param p
	 *            TreeNode for the TreeModel
	 */
	public void display(DefaultTreeModel model, MutableTreeNode p) {
		...
		if (nodes != null) {
			...
			for (int i = 0; i < nodes.size(); i++) {
				...
				// We create a new TreeNode composed of the pair and the list of
				// appearances
				DefaultMutableTreeNode currentNode = new DefaultMutableTreeNode(node.getPair()
						.toString() + " (" + child.getSupport() + ")");
