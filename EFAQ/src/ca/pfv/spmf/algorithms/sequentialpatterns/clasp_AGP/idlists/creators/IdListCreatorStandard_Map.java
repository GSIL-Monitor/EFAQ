...
public class IdListCreatorStandard_Map implements IdListCreator {
	...
	/**
	 * It adds to an Idlist of entries of arraylists several appearances in a
	 * same sequence <sid, {<tid_1,item1 position>,<tid_2, item2 position>,
	...
	public void initializeMaps(Map<Item, TrieNode> frequentItems,
			Map<Item, Map<Integer, List<Integer>>> projectingDistance,
		...
		for (Item frecuente : frequentItems.keySet()) {
			...
			Map<Integer, List<Integer>> sequenceElementsProjectingByItemMap = projectingDistance
					.get(frecuente);
			int totalNumberOfProjectedElements = 0;
			for (Integer s : sequenceElementsProjectingByItemMap.keySet()) {
				List<Integer> elementsProjectedByPatternInSequence = sequenceElementsProjectingByItemMap
						.get(s);
				for (int x : elementsProjectedByPatternInSequence) {
					totalNumberOfProjectedElements += sequenceSize.get(s) - x;
				}
			}
			node.getChild().getIdList()
					.setTotalElementsAfterPrefixes(totalNumberOfProjectedElements);
		}
	...
	@Override
	public void updateProjectionDistance(Map<Item, Map<Integer, List<Integer>>> projectingDistance,
			Item item, int id, int itemsetCount, int itemsCount) {
