...
public class IDListStandard_Map implements IDList {
	 ...
	 * 
	 * @param idList
	 *            IdList with which we join the current IdList.
	 * @param equals
	 *            Flag indicating if we want a intersection for equal relation,
	...
	public IDList join(IDList idList, boolean equals, int minSupport) {
		...
		Map<Integer, List<Position>> intersection = new HashMap<Integer, List<Position>>(
		...
		for (Map.Entry<Integer, List<Position>> entry : entries) {
			...
			/*
			 * We get the positions that correspond with the sequence given by
			 * the key of the current entry
			 */
			List<Position> positionAppearancesInSequence = entry.getValue();
			/*
			 * We create a new list of positions where we keep the result for
			 * this entry
			 */
			...
			if (equals) {
				...
				positionAppearances = equalOperation(sid, positionAppearancesInSequence,
						newTotalElementsAfterPrefixes);
			}
			else {
				// otherwise, we make an after operation join for the current
				// sequence sid
				positionAppearances = laterOperation(sid, positionAppearancesInSequence,
						newTotalElementsAfterPrefixes);
			}
		...
		// Finally, we return the new IdList and the sequence bitset associated
		// with it
	 ...
	 * 
	 * @param sid
	 *            Sequence identifier of the sequence where we want to check if
	 *            it exists the pattern
	 * @param positionAppearancesInSequence
	 *            Position items of the parameter Idlist
	 * @param dif
	 *            Place where we store the difference between the original size
	 *            of the sequence and the elements that there are up to the last
	 *            item appearance of the pattern that the IdList is referring to
	 * @return The new Entry for the new IdList
	 */
	private List<Position> laterOperation(Integer sid,
			List<Position> positionAppearancesInSequence, int[] dif) {
		// We get the positions for the same sequence for the current IdList
		List<Position> positionItemsAppearancesInSequenceOfMyIdList = sequencePositionsEntries
				.get(sid);
		...
		// Otherwise we create a new List of position items where we keep the
		// new entries
	 ...
	 * @return The new Entry for the new IdList
	 */
	private List<Position> equalOperation(Integer key,
			List<Position> positionItemsAppearancesInSequence, int[] dif) {
		// We get the position items for the same sequence for the current
		// IdList
		List<Position> positionItemsAppearancesInSequenceOfMyIdList = sequencePositionsEntries
				.get(key);
		// If there is not any occurrence we end the join operation
		...
		if (positionItemsAppearancesInSequenceOfMyIdList.size() <= positionItemsAppearancesInSequence
				.size()) {
	...
	/**
	 * It adds an appearance for the sequence and a position item given as
	 * parameter in the current IdList
	 * 
	 * @param sequence
	 *            Sequence identifier where the appearence occurs
	 * @param positionItem
	 *            Itemset timestamp where the appearance occurs
	 */
	...
	/**
	 * It adds the appearances for the sequence and the position items list
	 * given as parameter in the current IdList
	 * 
	 * @param sid
	 *            Sequence identifier where the appearence occurs
	 * @param itemsets
	 *            Itemset timestamps where the appearances occur
	 */
	...
	/**
	 * It set in the Trie object, given as parameter, the sequence identifiers
	 * where the pattern associated with the IdList appears
	 * 
	 * @param trie
	 */
	@Override
	...
	/**
	 * It returns the number of elements that appears after each appearance of
	 * the pattern associated with the IdList
	 * 
