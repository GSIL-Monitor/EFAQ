...
public class FrequentPatternEnumeration_ClaSP {
	...
	public FrequentPatternEnumeration_ClaSP(AbstractionCreator abstractionCreator,
			double minSupAbsolute, Saver saver, boolean findClosedPatterns,
	...
	public void dfsPruning(Pattern patron, Trie trie, boolean verbose,
			Map<Integer, Map<Integer, Integer>> coocMapAfter,
	...
	private void exploreChildren(Pattern pattern, TrieNode currentNode,
			List<TrieNode> sequenceExtensions, List<TrieNode> itemsetsExtensions, int beginning,
			Map<Integer, Map<Integer, Integer>> coocMapAfter,
		...
		for (TrieNode node : sequenceExtensions) {
			 ...
			 */
			joinCount++;
			IDList newIdList = currentTrie.getIdList().join(node.getChild().getIdList(), false,
					(int) minSupAbsolute);
		...
		for (int i = 0; i < sequenceExtensionSize; i++) {
			...
			Item last = newPattern.getIthElement(newPattern.size() - 1).getItem(); // PFV
																					// 2013
			 ...
			 * element
			 */
			exploreChildren(newPattern, nodeToExtend, new_sequenceExtension, new_sequenceExtension,
		...
		for (int k = beginning; k < itemsetsExtensions.size(); k++) {
			...
			ItemAbstractionPair newPair = ItemAbstractionPairCreator.getInstance()
					.getItemAbstractionPair(eq.getPair().getItem(),
		...
		for (int i = 0; i < itemsetExtensionSize; i++) {
			 ...
			 * element
			 */
			exploreChildren(newPattern, nodeToExtend, new_sequenceExtension, new_itemsetExtension,
					i + 1, coocMapAfter, coocMapEquals, last);
	 ...
	 * 
	 * @param prefix
	 *            Current pattern which is going to be checked
	 * @param trie
	 *            Trie associated with prefix
	 * @return
	 */
	private boolean isAvoidable(Pattern prefix, Trie trie) {
		...
		/*
		 * If there is not any pattern with the same key2 value, we add the
		 * current prefix as a new entry, and we also insert it in the matching
		 * map
		 */
		if (associatedMap == null) {
			 ...
			 * (and extensively with the same key1 value) we check if we can
			 * apply backward subpattern or backward superpattern pruning
			 */
			...
			if (associatedList == null) {
				...
				for (i = 0; i < associatedList.size(); i++) {
					...
					// If the support of the current prefix and the p pattern
					// are equal
						...
						if (pSize != prefixSize) {
							...
							if (prefixSize < pSize) {
								...
								if (prefix.isSubpattern(abstractionCreator, p)) {
									/*
									 * We dfsPruning backward subpattern pruning
									 * and establish as new nodes the nodes of
									 * the trie of p
									 */
								...
								/*
								 * if, conversely, the prefix size is greater
								 * than the size of p and prefix is a
								 * superpattern of p
								 */
								...
								superPattern++;
								/*
								 * and we make the prefix trie point to the
	...
	private int key2(IDList idlist, Trie t) {
		...
		// return FrequentPatternEnumeration_ClaSP.key_standardAndSumIDs(idlist,
		// t);
	}
	 ...
	 * 
	 * @param idList
	 *            IdList of the prefix to consider
	 * @return
	 */
	 ...
	 *            IdList of the prefix to consider
	 * @param trie
	 *            Trie of the pattern to consider
	 * @return
	 */
	 ...
	 *            List of patterns from which we want to remove the non-closed
	 *            patterns
	 * @param keepPatterns
	 *            Flag indicating if we want to keep the final output
	 */
	void removeNonClosedPatterns(List<Entry<Pattern, Trie>> frequentPatterns, boolean keepPatterns) {
		System.err.println("Before removing NonClosed patterns there are "
