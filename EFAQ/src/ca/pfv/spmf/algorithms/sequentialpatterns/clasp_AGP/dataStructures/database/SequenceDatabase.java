...
public class SequenceDatabase {
	...
	/**
	 * Map where, for each sequence, we have a list of integers corresponding to
	 * all the sizes of all the itemsets that the sequence has
	 */
	 ...
	 * elements after item>. We will use this map in order to maintain the
	 * values necessaries for making the pruning methods.
	 */
	...
	public void addSequence(String[] integers) {
		...
		for (int i = beginning; i < integers.length; i++) {
			if (integers[i].codePointAt(0) == '<') { // Timestamp
				...
				// insertMatchItemsetTimestamp(nSequences, sequence.size(),
				// timestamp);
				...
				if (indexParentheseGauche != -1) {
					...
					if (node == null) {
						...
						node = new TrieNode(pairCreator.getItemAbstractionPair(item,
								abstractionCreator.createDefaultAbstraction()), new Trie(null,
								idlist));
					...
					idListCreator.updateProjectionDistance(projectingDistance, item,
							sequence.getId(), sequence.size(), sequence.length() + itemset.size());
				}
			}
		}
	}
	...
	/**
	 * Get the equivalence classes associated with the frequent items that we
	 * have found.
	 * 
	 * @return the trie
	 */
