...
public class AlgoCloSpan {
	...
	/**
	 * Standard constructor. It takes the relative minimum support threshold
	 * (from 1 up to 0) and an abstraction creator
	 * 
	 ...
	 *            flag to indicate if we are interesting in only finding the
	 *            closed sequences
	 */
	public AlgoCloSpan(double minSupRelative, AbstractionCreator creator,
	 ...
	 * 
	 * @param database
	 *            The original database in which we apply PrefixSpan
	 * @param keepPatterns
	 *            Flag indicating if the user want to keep the frequent patterns
	 *            or he just want the amount of them
	 * @param verbose
	 *            Flag for debugging purposes
	 * @param outputFilePath
	 *            Path pointing out to the file where the output, composed of
	 *            frequent patterns, has to be kept. If, conversely, this
	 *            parameter is null, we understand that the user wants the
	 *            output in the main memory
	 * @throws IOException
	 */
	public void runAlgorithm(SequenceDatabase database, boolean keepPatterns, boolean verbose,
		...
		cloSpan(database, keepPatterns, verbose, findClosedPatterns, executePruningMethods,
				outputFilePath);
	 ...
	 * converted to a pseudosequece database, removing the infrequent items that
	 * appeared in the original dabase
	 * 
	 * @param database
	 ...
	 * @throws IOException
	 */
	protected void cloSpan(SequenceDatabase database, boolean keepPatterns, boolean verbose,
			boolean findClosedPatterns, boolean executePruningMethods, String outputFilePath)
			throws IOException {
		 ...
		 * pseudosequence database
		 */
		PseudoSequenceDatabase pseudoDatabase = projectInitialDatabase(database, mapSequenceID,
				(int) minSupAbsolute);

		// We initialize the class that is in charge of managing the main loop
		// of PrefixSpan
		RecursionCloSpan algorithm = new RecursionCloSpan(abstractionCreator, saver,
				(int) minSupAbsolute, pseudoDatabase, mapSequenceID, findClosedPatterns,
		...
		if (verbose) {
			System.out.println("CLOSPAN: The algorithm takes " + timeForMainMethod
					+ " seconds and finds " + numberOfFrequentPatterns + " patterns");
		}
		// If the we are interested in closed patterns, we execute the
		...
		if (findClosedPatterns) {
			...
			if (verbose) {
				System.out
						.println("CLOSPAN: The post-processing algorithm to remove the non-Closed patterns takes "
								+ timeForPostProcessingStep
								+ " seconds and finds "
								+ numberOfFrequentPatterns + " Closed patterns");
			}
		}
		else {
			if (keepPatterns) {
	 ...
	 *            The original Database
	 * @param mapSequenceID
	 *            Map with all the items appearing in the original database, and
	 *            a bitset pointing out in which sequences the items appear
	 * @param minSupportAbsolute
	 *            The absolute minimum support
	 * @return
	 */
	private PseudoSequenceDatabase projectInitialDatabase(SequenceDatabase database,
			Map<Item, BitSet> mapSequenceID, long minSupportAbsolute) {
		PseudoSequenceDatabase initialContext = new PseudoSequenceDatabase();
		// For each database sequence
		for (Sequence sequence : database.getSequences()) {
			// The new pseudosequences are optimized, since do not have the
			// infrequent items
			...
			if (optimizedSequence.size() != 0) {
				 ...
				 * of an original sequence, we insert an empty pseudosequence in
				 * order not to affect to the absolute minimum support
				 */
