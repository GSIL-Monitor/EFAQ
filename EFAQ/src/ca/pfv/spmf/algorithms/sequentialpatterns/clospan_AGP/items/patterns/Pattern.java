...
public class Pattern implements Comparable<Pattern> {
	...
	public int compareTo(Pattern o) {
		...
		for (int i = 0; i < elementOfSmallerPattern.size(); i++) {
			int comparison = elementOfSmallerPattern.get(i).compareTo(
					elementsOfGreaterPattern.get(i));
			if (comparison != 0) {
				return comparison;
			}
		}
	 ...
	 *            an instance of an abstraction creator
	 * @param p
	 *            the other pattern
	...
	/**
	 * Get the sum of all the sequence identifiers that are present in the
