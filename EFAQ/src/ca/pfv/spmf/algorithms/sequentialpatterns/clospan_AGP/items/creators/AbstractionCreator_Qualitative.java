...
public class AbstractionCreator_Qualitative extends AbstractionCreator {
	 ...
	 * 
	 * @param equalRelation
	 *            The boolean indicatin if the item has an equal relation with
	 ...
	 * that pair. If the pair has not been previously kept, we keep the
	 * sequenceID
	 * 
	 * @param pairMap
	 *            a map of Pair object
	 * @param alreadyCountedForSequenceID
	 *            the set of sequence IDs that have been already counted
	 * @param id
	 *            an ID
	 * @param item
	 *            the item
	 * @param postfix
	 *            indicates if it is the case of a postfix (an itemset cut at
	 *            left)
	 */
	private void addPair(Map<Pair, Pair> pairMap, Set<Pair> alreadyCountedForSequenceID, int id,
			Item item, boolean postfix) {
		/*
		 * We create a new Pair object from the given item and the postfix flag
		 */
		Pair pair = new Pair(postfix, ItemAbstractionPairCreator.getInstance()
				.getItemAbstractionPair(item, createAbstraction(postfix)));
	...
	/**
	 * Method to find all frequent items in a context (database). This is for k>
	 * 1.
	 * 
	 * @param sequences
	 *            the list of pseudosequences from the database
	...
	public Set<Pair> findAllFrequentPairs(List<PseudoSequence> sequences) {
		// we will scan the database and store the cumulative support of each
		// pair in a map.
		...
		for (PseudoSequence sequence : sequences) {
			...
			for (int k = 0; k < sequence.numberOfProjectionsIncluded(); k++) {
				for (int i = 0; i < sequence.size(k); i++) {
					// If we are after the first projection and after the first
					// itemset of the pseudosequence
					if (k > 0 && i > 0) {
					...
					// We obtain the beginning of that itemset for our
					// projection
					...
					for (int j = beginning; j < itemset.size(); j++) {
						...
						addPair(pairMap, alreadyCountedForSequenceID, sequence.getId(), item,
								postfix);
					}
				}
			}
	 ...
	 *            the sequence
	 * @param frequentItems
	 *            a map of frequent item and their corresponding bitsets.
	 * @return the resulting map
	 */
	@Override
	public Map<Item, Set<Abstraction_Generic>> createAbstractions(Sequence sequence,
			Map<Item, BitSet> frequentItems) {
		return new HashMap<Item, Set<Abstraction_Generic>>();
	}

	/**
	 * It creates an abstraction from a prefix and it is the same abstraction
	 ...
	 */
	@Override
	public Abstraction_Generic createAbstractionFromAPrefix(Pattern prefix,
			Abstraction_Generic abstraccion) {
		return abstraccion;
	}
	 ...
	 * 
	 * @param shorter
	 *            The pattern which we check if is a subpattern of another
	 *            longer than it
	 * @param larger
	 *            Pattern which we want to check if another pattern is
	 *            subpattern of itself
	 * @param index
	 *            index that indicates which position we have to take into
	 *            account
	 * @param positions
	 *            List of positions of the appearances of the elements of the
	...
	public boolean isSubpattern(Pattern shorter, Pattern larger, int index, List<Integer> positions) {
		...
		Abstraction_Generic previousAbs = index > 0 ? shorter.getIthElement(index - 1)
				.getAbstraction() : null;
		...
		while (positions.get(index) < larger.size()) {
			...
			if (index == 0) {
				...
				pos = findItemPositionInPattern(larger, itemPair, absPair, previousAbs,
						positions.get(index), positions.get(index - 1));
			}
			...
			if (pos != null) {
				...
				if (index + 1 < shorter.size()) {
					...
					// And we initialize the next index position to that new
					// position
				...
				else {// If, conversely, we are in the last element of the
						// shorter pattern
					/*
			...
			else {// If conversely, we did not find any position for the current
					// index
				...
				// We set to to true the flag that indicates the end of the
				// method
				cancelled = true;
				/*
		...
		/*
		 * If we are finish the loop and not by breaking it, and we are not
		 * looking for the first element of the shorter pattern
		 */
	 ...
	 * 
	 * @param p
	 *            Pattern where we search for an item
	 * @param beginning
	 *            Index from which we start to search from the item
	 * @param itemPair
	 *            Item to search for
	 * @return The item position where we found the item, or null if this does
	 *         not appear
	 */
	...
	/**
	 * It searches for a position in the pattern given as parameter where an
	 * item, also given as a parameter, appears
	 * 
	 * @param p
	 *            Pattern where we are going to search for
	 * @param itemPair
	 *            Item to search for
	 * @param currentAbs
	 *            Abstraction of the current element of the pattern where the
	 *            item appeared
	 * @param previousAbs
	 *            Astraction of the previous element of the pattern where the
	 *            item appeared
	 ...
	 * @return the position
	 */
	public Integer findItemPositionInPattern(Pattern p, Item itemPair,
			Abstraction_Generic currentAbs, Abstraction_Generic previousAbs,
			Integer currentPosition, Integer previousPosition) {
		Abstraction_Qualitative abs = (Abstraction_Qualitative) currentAbs;
		Integer pos = null;
		// If the current Abstraction has an equal relation with the previous
		// pair
		if (abs.hasEqualRelation()) {
			// We search for the item in the same itemset where the previous
			// item appeared
	 ...
	 * 
	 * @param p
	 *            Pattern in which we search for the beginning of another
	 *            itemset
	 * @param beginning
	 *            Index from which we start to search for
	...
	public int increaseItemset(Pattern p, Integer beginning) {
		...
		for (int i = beginning + 1; i < p.size(); i++) {
			...
			Abstraction_Qualitative qualitativeAbs = (Abstraction_Qualitative) currentPair
					.getAbstraction();
			// If the relation is not an equal relation, then we have changed of
			// itemset
	 ...
	 * 
	 * @param pattern
	 *            Pattern where we are goin to search for the item
	 * @param itemPair
	 *            Item to search for
	 * @param beginning
	 *            Index from which we are going to start to search for
	...
	private Integer searchForInTheSameItemset(Pattern pattern, Item itemPair, Integer beginning) {
		...
		for (int i = beginning; i < pattern.size(); i++) {
			...
			Abstraction_Qualitative qualitativeAbstraction = (Abstraction_Qualitative) currentPair
					.getAbstraction();
	 ...
	 * 
	 * @param pattern
	 *            Pattern in which we check the two positions
	 * @param p1
	 *            First position
	 * @param p2
	 *            Second position
