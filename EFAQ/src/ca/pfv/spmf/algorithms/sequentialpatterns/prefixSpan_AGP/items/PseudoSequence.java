...
public class PseudoSequence {
	...
	/**
	 * List with the corresponding timeshift that each projection in the
	 * sequence has
	 */
	 ...
	 * 
	 * @param timeShift
	 *            The time shift of the new pseudosequence
	 * @param pseudosequence
	 *            The pseudosequence where we make another projection
	 * @param itemsetIndex
	 *            The itemset index where our new projection is starting
	 * @param itemIndex
	 *            The item index where our new projection is starting
	 * @param firstItemset
	 *            An index that indicates in which projection of the previous
	 *            pseudoseuqence we are making the new pseudosequence
	 */
	public PseudoSequence(long timeShift, PseudoSequence pseudosequence, int itemsetIndex,
			int itemIndex, int firstItemset) {
	...
	/**
	 * PseudoSequence made from a starndard sequence. Its purpose is make
	 * possible the later projections
	 * 
	 * @param timeShift
	 *            The original timeshift (Normally, it will be 0)
	 * @param sequence
	 *            The original standard sequence
	 * @param itemsetIndex
	 *            The itemset index where the projection it starts
	 * @param itemIndex
	 *            The item index where the projection it starts
	 */
	...
	/**
	 * Method to make a projection in a pseudosequence already projected. In
	 * this way, for a same pseudosequence, we can have several projections. For
	 * example, let us think in a sequence A = (a) (a b) (a b c). If we project
	 * by item a, we obtain three different projections: 1) (a b) (a b c) 2)
	 * (*b) (a b c) 3) (*b c) That we do is to put all the three projections in
	 * the same pseudoseuqence, and we refers each projection with an index. We
	 * call to every projection, projection point.
	 * 
	 * @param firstItemset
	 *            The projection point of the given pseudosequence where we are
	 *            making our new projection
	 * @param timeShift
	 *            The given timeshift for our projection
	 * @param pseudosequence
	 *            The given pseudosequence where we are making our projection
	 * @param itemsetIndex
	 *            The itemset index where our projection it starts
	 * @param itemIndex
	 *            The item index where our projection it starts
	 */
	public void addProjectionPoint(int firstItemset, long timeShift, PseudoSequence pseudosequence,
			int itemsetIndex, int itemIndex) {
		/*
		...
		this.timeShift.add(newTimeShift);
		/*
		 * We establish the starting itemset index (for our projection), from
		 * the previous starting itemset index
		 */
		this.firstItemset.add(itemsetIndex + pseudosequence.firstItemset.get(firstItemset));
		// If our itemset index is the same index in the previous pseudosequence
		if (this.firstItemset.get(this.firstItemset.size() - 1) == pseudosequence.firstItemset
				.get(firstItemset)) {
	 ...
	 * 
	 * @param index
	 *            the ith projection in which we want to know the first itemset
	 ...
	 * 
	 * @param i
	 *            the ith projection in which we want to know the size
	 * @return the size
	 */
	 ...
	 * 
	 * @param firstItemset
	 *            the ith projection in which we are interested
	 * @param index
	 *            The ith itemset of the current pseudosequence
	 * @return the size
	 */
	...
	/**
	 * It returns the first item of the first itemset of the specified
	 * projection
	 * 
	 * @param firstItem
	 *            the specified projection in which we are interested
	 * @param itemsetIndex
	 *            the itemset of the pseudosequence in which we are interested
	 * @return the item
	 */
	 ...
	 * 
	 * @param firstItem
	 *            The projection in which we are interested
	 * @param itemsetIndex
	 *            The itemset to study
	 ...
	 * 
	 * @param firstItem
	 *            The projection in which are interested
	 * @param itemIndex
	 *            The item index
	 * @param itemsetIndex
	 *            The itemset index
	...
	public Item getItemAtInItemsetAt(int firstItem, int itemIndex, int itemsetIndex) {
		...
		if (isFirstItemset(itemsetIndex)) {
			return getItemset(itemsetIndex, firstItem).get(
					itemIndex + this.firstItem.get(firstItem));
		}
		else {
	 ...
	 * 
	 * @param itemsetIndex
	 *            the itemset index that we want to know its timestamp
	 * @param firstItemset
	 *            The projection of the pseudosequence in which we are
	 *            interested
	 * @return
	 */
	 ...
	 * 
	 * @param itemsetIndex
	 *            the itemset index for the pseudosequence
	 * @param firstItemset
	 *            the projection in which we are interested
	...
	/**
	 * It returns the index where an item appears in the specified itemset of
	 * the specified projection of the current pseudosequence. If the item does
	 * not appear, it returns -1.
	 * 
	 * @param firstItemset
	 *            the index of the firstItemset to search
	 * @param itemsetIndex
	 *            the current itemset index
	 * @param item
	 *            the item
