...
class RecursionPrefixSpan_AGP {
	 ...
	 * @param saver
	 *            The saver for correctly save the results where the user wants
	 * @param minSupportAbsolute
	 *            The absolue minimum support
	 * @param pseudoDatabase
	 *            The original pseudoSequence database (without frequent items)
	 * @param mapSequenceID
	 *            Map which match the frequent items with their appearances
	 */
	public RecursionPrefixSpan_AGP(AbstractionCreator abstractionCreator, Saver saver,
			long minSupportAbsolute, PseudoSequenceDatabase pseudoDatabase,
			Map<Item, BitSet> mapSequenceID) {
	 ...
	 * 
	 * @param keepPatterns
	 *            Flag indicating if the user wants to keep the results or he is
	 *            just interested in the number of frequent patterns
	 * @param verbose
	...
	public void execute(boolean keepPatterns, boolean verbose) {
		...
		for (Item item : keySetList) {
			cont++;
			if (verbose) {
				System.out.println("Projecting item = " + item + " (" + cont + "/"
			...
			ItemAbstractionPair pair = new ItemAbstractionPair(item,
					abstractionCreator.CreateDefaultAbstraction());
	 ...
	 * 
	 * @param item
	 *            The item from which we make the projection
	 * @param database
	 *            The database where we make the projection
	 * @param abstraction
	 *            Abstraction associated with the item to project
	 * @param firstTime
	 *            Flag that points out if it the first time that
	 * @return The new projected database
	 */
	private PseudoSequenceDatabase makePseudoProjections(Item item,
			PseudoSequenceDatabase database, Abstraction_Generic abstraction, boolean firstTime) {
		...
		for (int sequenceIndex = 0; sequenceIndex < pseudoSequences.size(); sequenceIndex++) { // for
																								// each
																								// sequence
			 ...
			 */
			int potentialSize = newProjectedDatabase.size() + pseudoSequences.size()
					- sequenceIndex;
			if (potentialSize < minSupportAbsolute) {
				return null;
			}
			/*
			 * Flag indicating if the current sequence has already been
			 * projected for the new projected database
			 */
			...
			// Initialization of the number of projections done in the current
			// sequence
			...
			for (int k = 0; k < sequence.numberOfProjectionsIncluded(); k++) {
				...
				for (int i = 0; i < sequenceSize; i++) {
					// we get the index ofthe given item to project in current
					// the itemset
					int index = sequence.indexOf(k, i, item);
					// If the item has been found and either is the first
					// projection or the method compute is true
					if (index != -1 && (firstTime || (abstraction.compute(sequence, k, i)))) {
						...
						if (index != itemsetSize - 1) {
							...
							if (!alreadyProjected) {
								// A new pseudosequence is created starting from
								// the next point to the found item
								newSequence = new PseudoSequence(
										sequence.getRelativeTimeStamp(i, k), sequence, i,
										index + 1, k);
								// We keep the projection point
								projectionsAlreadyMade.add(sequence.getFirstItemset(k) + i);
								// If the new pseudosequence has more than one
								// item
								if (newSequence.size(numberOfProjections) > 0) {
									// we increase the number of projections
									numberOfProjections++;
									// And we add the new projected sequence to
									// the new database
								...
								if (projectionsAlreadyMade.add(sequence.getFirstItemset(k) + i)) {
									 ...
									 */
									newSequence.addProjectionPoint(k,
											sequence.getRelativeTimeStamp(i, k), sequence, i,
											index + 1);
								}
							}
							/*
							 * if the found item is the last item of the
							 * sequence and the item where it is, it is not the
							 * last itemset of the sequence
							 */
						}
							...
							if (!alreadyProjected) {
								 ...
								 * in the next itemset to where the item
								 * appeared
								 */
								newSequence = new PseudoSequence(
										sequence.getRelativeTimeStamp(i, k), sequence, i + 1, 0, k);
								...
								if (projectionsAlreadyMade.add(sequence.getFirstItemset(k) + i)) {
									 ...
									 */
									newSequence
											.addProjectionPoint(k,
													sequence.getRelativeTimeStamp(i, k), sequence,
													i + 1, 0);
								}
							}
						}
	 ...
	 * 
	 * @param prefix
	 *            prefix from which we made the projected database and where the
	 *            frequent items that we find will be added
	 * @param k
	 *            size of patterns that are going to be generated
	 * @param context
	 *            prefix-projected databases
	 * @param keepPatterns
	 *            flag indicating if we want to keep the output or we are
	 *            interesting in just the number of frequent patterns
	 * @param verbose
	 *            flag for debuggin purposes
	 */
	private void prefixSpanLoop(Pattern prefix, int k, PseudoSequenceDatabase context,
			boolean keepPatterns, boolean verbose) {
		...
		if (verbose) {
			...
			System.out.print(tab + "\tFound " + pairs.size()
					+ " frequent items in this projection\n");
		}
		// For each pair found,
		for (Pair pair : pairs) {
			...
			if (pair.getSupport() >= minSupportAbsolute) {
				...
				ItemAbstractionPair newPair = pairCreator.getItemAbstractionPair(pair.getPair()
						.getItem(), abstractionCreator.createAbstractionFromAPrefix(prefix, pair
						.getPair().getAbstraction()));
				newPrefix.add(newPair);
				// build the projected database with respect to this frequent
				// item (the item which forms the prefix)
				PseudoSequenceDatabase projection = makePseudoProjections(pair.getPair().getItem(),
						context, pair.getPair().getAbstraction(), false);
				// We add its set of sequences where the prefix appear
				newPrefix.setAppearingIn((BitSet) (pair.getSequencesID().clone()));
				// if the flag of keeping patterns if active, we keep this new
				// pattern
				if (keepPatterns) {
				...
				numberOfFrequentPatterns++;
				// If the projection exists and has more sequences than the
				// absolute minimum support
