...
/***
 * This is an implementation of the TKS algorithm for top-k sequential patterns. <br/>
 * <br/>
 * 
...
public class AlgoTKS {
	...
	private long						startTime;																						// start
																																		...
																																		// algorithm
																																		// execution
	private long						startMiningTime;																				// end
																																		// time
																																		// of
																																		// preprocessing
																																		// of
	...
	PriorityQueue<Pattern>				kPatterns;																						// the
																																		// top
																																		// k
																																		// patterns
																																		// found
																																		// until
																																		// now
	PriorityQueue<Candidate>			candidates;																					// the
																																		// candidates
																																		// for
																																		// expansion
	...
	// the set of items that have been discarded because their support has
	// become
	 ...
	 * @param k
	 *            the number of sequential pattern to be found
	 */
	public PriorityQueue<Pattern> runAlgorithm(String input, String outputFilePath, int k)
			throws IOException {
	 ...
	 *            input file
	 * @param minsupRel
	 *            the minimum support as a relative value
	 * @throws IOException
	 */
	private PriorityQueue<Pattern> tks(String input, int k) throws IOException {
		...
		// STEP 0: SCAN THE DATABASE TO STORE THE FIRST BIT POSITION OF EACH
		// SEQUENCE
		// AND CALCULATE THE TOTAL NUMBER OF BIT FOR EACH BITMAP
		sequencesSize = new ArrayList<Integer>();
		lastBitIndex = 0; // variable to record the last bit position that we
							// will use in bitmaps
		try {
			// read the file
			...
			while ((thisLine = reader.readLine()) != null) {
				...
				// kind of metadata
				if (thisLine.isEmpty() == true || thisLine.startsWith("#")
				...
				for (int i = 0; i < tokens.length; i++) {
					...
					if (item == -1) { // indicate the end of an itemset
						// increase the number of bits that we will need for
						// each bitmap
						bitIndex++;
					}
				}
		...
		// record start time
		startTime = System.currentTimeMillis(); // FOR COMPARISON WITH PPWSR WE
												// HAVE TO PUT HERE
		...
		for (int[] transaction : inMemoryDB) {
			...
			for (Integer item : transaction) {
				...
				// determineSection(bitindex - previousBitIndex); // register
				// the sequence length for the bitmap
					sid++;
					tid = 0;
				}
		...
		// // STEP2: REMOVE INFREQUENT ITEMS FROM THE DATABASE BECAUSE THEY WILL
		// NOT APPEAR IN ANY FREQUENT SEQUENTIAL PATTERNS
		List<Integer> frequentItems = new LinkedList<Integer>();
		Iterator<Entry<Integer, Bitmap>> iter = verticalDB.entrySet().iterator();
		// we iterate over items from the vertical database that we have in
		// memory
		while (iter.hasNext()) {
		...
		// PRUNE INFREQUENT ITEMS FROM VERTICAL DB AND REMOVE ITEM FROM LIST OF
		// FREQUENT ITEMS
		...
		if (useCooccurrenceInformation) {
			...
			while (iter2.hasNext()) {
				...
				if (bitmap.getSupport() >= minsup) {
					...
					if (coocMapAfter.get(item) != null) {
						...
						registerAsCandidate(new Candidate(prefix, bitmap, afterItems, afterItems,
								item));
					}
				}
			}
			...
			while (iter2.hasNext()) {
				...
				if (verticalDB.get(item).getSupport() < minsup) {
					...
					prefix.addItemset(new Itemset(item));
					// We register this prefix as a path for future exploration
					registerAsCandidate(new Candidate(prefix, verticalDB.get(item), frequentItems,
							frequentItems, item));
				}
			}
		}
		...
		if (kPatterns.size() > k) {
			...
			if (pattern.support > this.minsup) {
				...
				do {
					...
					// STRATEGY TO RECORD DISCARDED SINGLE ITEMS SO THAT WE
					// DON'T CONSIDER THEM FOR EXPANSION
					if (useDiscardedItemsPruningStrategy && pat.prefix.size() == 1
							&& pat.prefix.get(0).size() == 1) {
	 ...
	 *            the current prefix
	 * @param prefixBitmap
	 *            the bitmap corresponding to the current prefix
	 * @param sn
	 *            a list of items to be considered for i-steps
	 * @param in
	 *            a list of items to be considered for s-steps
	 * @param hasToBeGreaterThanForIStep
	 * @param m
	 *            size of the current prefix in terms of items
	 * @throws IOException
	 *             if there is an error writing a pattern to the output file
	 */
	private void dfsPruning(Prefix prefix, Bitmap prefixBitmap, Collection<Integer> sn,
			Collection<Integer> in, int hasToBeGreaterThanForIStep) throws IOException {
		// System.out.println(prefix.toString());
		// Itemset lastItemsetOfPrefix =
		// prefix.get(prefix.getItemsets().size()-1));
		// Integer lastAppendedItem =
		// lastItemsetOfPrefix.get(lastItemsetOfPrefix.size()-1);
		...
		// Map<Integer, Integer> mapSupportItemsAfter =
		// coocMapAfter.get(lastAppendedItem);
		...
		for (Integer i : sn) {
			...
			Bitmap newBitmap = prefixBitmap.createNewBitmapSStep(verticalDB.get(i), sequencesSize,
					lastBitIndex);
		...
		for (Integer i : in) {
			...
			Bitmap newBitmap = prefixBitmap.createNewBitmapIStep(verticalDB.get(i), sequencesSize,
					lastBitIndex);
