...
/**
 * Implementation of a bitmap for SPAM. <br/>
 * <br/>
 * 
 * Copyright (c) 2008-2012 Philippe Fournier-Viger <br/>
 * <br/>
 * 
...
public class Bitmap {
	...
	int					firstItemsetID		= -1;			// the id of the
															// first itemset
															// containing a bit
															// set to 1 (in any
															// sequence)

	private int			support				= 0;			// the number of
															// bits that are
															// currently set to
															// 1
	 ...
	 * 
	 * @param lastBitIndex
	 *            the desired size of the bitset minus 1
	 */
	 ...
	 * 
	 * @param bitmap
	 *            a bitset to initialize this Bitmap.
	 */
	 ...
	 * 
	 * @param sid
	 *            the sid corresponding to that bit
	 * @param tid
	 *            the tid corresponding to that bit
	 * @param sequencesSize
	 *            the list of sequence length to know how many bits are
	 *            allocated to each sequence
	 */
	 ...
	 * 
	 * @param bit
	 *            the position of the bit in the bitmap
	 * @param sequencesSize
	 *            the list of lengths of sequence by sequence ID.
	...
	/**
	 * Create a new bitmap for the s-step by doing a AND between this bitmap and
	 * the bitmap of an item.
	 * 
	 * @param bitmapItem
	 *            the bitmap of the item used for the S-Step
	 * @param sequencesSize
	...
	Bitmap createNewBitmapSStep(Bitmap bitmapItem, List<Integer> sequencesSize, int lastBitIndex) {
		...
		for (int bitK = bitmap.nextSetBit(0); bitK >= 0; bitK = bitmap.nextSetBit(bitK + 1)) {
			...
			boolean match = false;
			for (int bit = bitmapItem.bitmap.nextSetBit(bitK + 1); bit >= 0 && bit <= lastBitOfSID; bit = bitmapItem.bitmap
					.nextSetBit(bit + 1)) {
	...
	/**
	 * Create a new bitmap by performing the I-STEP with this bitmap and the
	 * bitmap of an item.
	 * 
	 * @param bitmapItem
