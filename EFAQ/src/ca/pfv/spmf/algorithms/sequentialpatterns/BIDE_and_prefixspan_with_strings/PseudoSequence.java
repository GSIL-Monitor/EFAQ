 ...
 * sequence starts and (2) at which item in this itemset the sequence starts. <br/>
 * <br/>
 * 
...
class PseudoSequence {
	 ...
	 * @param indexItemset
	 *            the itemset where the pseudo-sequence should start in terms of
	 *            the original sequence.
	 * @param indexItem
	 *            the item where the pseudo-sequence should start in terms of
	 *            the original sequence.
	 */
		...
		for (int i = 0; i < size(); i++) {
			...
			for (int j = 0; j < getSizeOfItemsetAt(i); j++) {
				if (!isLastItemset(i)) {
					...
					// if it is in a postfix, we add a "*" symbol beside the
					// item
					if (isPostfix(i)) {
						r.append('*');
					}
	 ...
	 * 
	 * @param indexItemset
	 *            the given itemset position
	 * @param idItem
	 *            the item that we want to search.
