...
public class FrequentPatternEnumeration {
	...
	public FrequentPatternEnumeration(CandidateGenerator candidateGenerator,
			double minSupportAbsolute, Saver saver) {
	 ...
	 * @param dfs
	 *            Flag indicating if we are interested in a depth-first search
	 *            if activated. Otherwise, we understand that we are interested
	...
	public void execute(EquivalenceClass eq, boolean dfs, boolean keepPatterns, boolean verbose,
		...
		/*
		 * eq.setIdList(null); eq.setClassIdentifier(null);
		 */
		...
		for (int i = eqMembers.size() - 1; i >= 0; i--) {
			...
			for (int j = i; j >= 0; j--) {
				...
				if (coocMapEquals != null) {
					Integer itemY = (Integer) child_Y.getClassIdentifier().getLastElement()
							.getItem().getId();
				 ...
				 */
				List<Pattern> extensions = candidateGenerator.generateCandidates(
						child_X.getClassIdentifier(), child_Y.getClassIdentifier(),
						(int) minSupportAbsolute, doNotExploreXY, doNotExploreYX, doNotExploreX_Y,
						doNotExploreY_X);
				// For each candidate generated
				for (Pattern extension : extensions) {
					IDList newIdList = candidateGenerator.join(extension, child_X, child_Y,
			...
			if (dfs) {
				/*
				 * We remove the current member pointed out by i index of the
				 * set of members of eq
				 */
				eqMembers.remove(i);
				/*
		...
		// If dfs is not activated, therefore we are interested in a
		...
		if (!dfs) {
			...
			if (anyPatternCreated) {
				for (int i = eqMembers.size() - 1; i >= 0; i--) {
					...
					execute(eqMembers.get(i), dfs, keepPatterns, verbose, coocMapAfter,
							coocMapEquals);
					/*
	 ...
	 * 
	 * @param eq
	 *            Equivalence class with we want to insert as a member of its
	 *            father
	 * @param eq_X
	 *            Candidate father 1
	 * @param eq_Y
	 *            Candidate father 2
	 */
	private void insertClassByPrefix(EquivalenceClass eq, EquivalenceClass eq_X,
			EquivalenceClass eq_Y) {
		...
		else {// Otherwise, eq_Y is prefix of eq and we insert eq as a member of
				// eq_Y
	 ...
	 * 
	 * @param pattern
	 *            The pattern that we want to keep.
	 */
