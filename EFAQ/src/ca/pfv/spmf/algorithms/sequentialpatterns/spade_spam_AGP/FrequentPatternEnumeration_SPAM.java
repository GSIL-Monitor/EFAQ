 ...
 * also have other two ones (both based on hash maps, one with bitsets
 * (IDListBitmap) and another with arraylists (IDListStandard_Map)).
 * 
...
public class FrequentPatternEnumeration_SPAM {
	 ...
	 * @param saver
	 *            Saver object to decide where the user want to save the
	 ...
	 *            The equivalence class from we start to search for.
	 * @param keepPatterns
	 *            Flag to indicate if we want to keep the patterns found.
	 * @param verbose
	...
	public void execute(EquivalenceClass equivalenceClass, boolean keepPatterns, boolean verbose) {
		...
		for (int i = 0; i < numberOfMembersInEC; i++) {
			...
			// We call to the main method of the algorithm for that equivalence
			// class
			dfs_pruning(ec, equivalenceClass.getClassMembers(), equivalenceClass.getClassMembers(),
					i + 1, keepPatterns);
		}
	}
	 ...
	 * tries to make a s-extension first, and then, once it found all the
	 * possible s-extension, it tries to make an i-extension. The method
	 * receives two set of equivalence classes, one with the frequent items that
	 * can be used for making s-extensions and another one for making
	 * i-extensions.
	 * 
	 * @param currentClass
	 *            The current class whose identifier we are trying to extend.
	 * @param sequenceExtensions
	 *            The set of equivalence classes, with frequent items as
	 *            identifiers, that we use as possible s-extensions of the
	 *            current pattern that we are processing.
	 * @param itemsetsExtensions
	 *            The set of equivalence classes, with frequent items as
	 *            identifiers, that we use as possible i-extensions of the
	 *            current pattern that we are processing.
	 * @param beginning
	 *            The beginning index from where we can use the elements of
	 *            itemsetsExtensions set
	 * @param keepPatterns
	 *            Flag indicating if we are interesting in saving the frequent
	 *            patterns that we find.
	 */
	private void dfs_pruning(EquivalenceClass currentClass,
			List<EquivalenceClass> sequenceExtensions, List<EquivalenceClass> itemsetsExtensions,
			int beginning, boolean keepPatterns) {
		...
		for (EquivalenceClass eq : sequenceExtensions) {
			...
			Pattern extension = patternCreator.createPattern(new ArrayList<ItemAbstractionPair>(
					clone.getElements()));
			 ...
			 */
			IDList newIdList = currentClass.getIdList().join(eq.getIdList(), false,
			...
			if (newIdList.getSupport() >= minSupportAbsolute) {
				...
				/*
				 * Normally, in the original algorithm we would insert in the
				 * newset of sequence extensions the same eq class that we used
				 * for obtaining the new pattern. The problem with this is that
				 * if we do it, the Idlist it remains the same. Since a IdList
				 * is denser (with more values) with shorter patterns (because
				 * is easier to find an appearance in a sequence), we can put to
				 * the pattern of the equivalence class of eq, the new idlist
				 * generated. Note that for the items in future s-extensions of
				 * this new pattern, we can directly pass the IdList recently
				 * computed since all the appearances of eq identifier will be
				 * after an appearance of the current class. Therefore, in order
				 * to shrink the IdLists and their computations we create a new
				 * equivalence class with the same eq identifier, and the idlist
				 * recently created.
				 */
		...
		for (int i = 0; i < sequenceExtensionSize; i++) {
			 ...
			 * extension. Besides we establish the same set as the set which we
			 * will make the i-extensions, but beginning from the (i+1)-th
			 * element
			 */
			dfs_pruning(newClass, new_sequenceExtension, new_sequenceExtension, i + 1, keepPatterns);
			/*
			 * Once we had finished the search for this patterns and their
			 * children, we can to remove that class (and its desdendants) from
			 * the memory
			 */
			newClass.clear();
		}
		...
		for (int k = beginning; k < itemsetsExtensions.size(); k++) {
			...
			// We create a new pattern with the elements of the current class
			// identifier
			Pattern extension = patternCreator.createPattern(new ArrayList<ItemAbstractionPair>(
					clone.getElements()));
			// And we add it the current item of itemset extension set
			ItemAbstractionPair newPair = ItemAbstractionPairCreator.getInstance()
					.getItemAbstractionPair(eq.getClassIdentifier().getLastElement().getItem(),
							AbstractionCreator_Qualitative.getInstance().createAbstraction(true));
			extension.add(newPair);
			 ...
			 * the appearances of the new pattern and its support.
			 */
			IDList newIdList = currentClass.getIdList().join(eq.getIdList(), true,
					(int) minSupportAbsolute);
			// If the new pattern is frequent
			if (newIdList.getSupport() >= minSupportAbsolute) {
				// We insert it its appearances
				...
				/*
				 * Normally, in the original algorithm we would insert in the
				 * newset of itemset extensions the same eq class that we used
				 * for obtaining the new pattern. The problem with this is that
				 * if we do it, the Idlist it remains the same. Since a IdList
				 * is denser (with more values) with shorter patterns (because
				 * is easier to find an appearance in a sequence), we can put to
				 * the pattern of the equivalence class of eq, the new idlist
				 * generated. Note that for the items in future i-extensions of
				 * this new pattern, we can directly pass the IdList recently
				 * computed since all the appearances of eq identifier will be
				 * after an appearance of the current class. Therefore, in order
				 * to shrink the IdLists and their computations we create a new
