...
public class IDListFatBitmap implements IDList {
	...
	final int				BIT_PER_SEQUENCE	= 512;	// the number of bit
														// that we use to code a
														// database
	final int				BIT_PER_ITEMSET		= 64;	// the number of bit
	 ...
	 * @param tid
	 *            The itemset where the pattern appears
	 */
	 ...
	 *            The sequence identifier where the pattern appears
	 * @param tids
	 *            The itemsets where the pattern appears
	 */
	 ...
	 *            method
	 * @param sequencesFromIdList
	 *            Sequence bitset with which we are going to join the current
	 *            IdList.
	 * @param itemsetsFromIdList
	 *            Itemsets bitset with which we are going to join the current
	 *            IdList.
	 * @param minSupport
	 *            Mininum relative support
	 */
	private void equalLoop(IDListFatBitmap newIdList, BitSet sequencesFromIdList,
			List<BitSet> itemsetsFromIdList, int minSupport) {
		...
		// If the new sequence bitset has a potential support at least as the
		// minimum support
		if (newIdList.getSupport() >= minSupport) {
			newIdList.setSequences(sequencesIntersection);
			// For each sequence appearance
			for (int i = sequencesIntersection.nextSetBit(0); i >= 0; i = sequencesIntersection
					.nextSetBit(i + 1)) {
				/*
	 ...
	 *            Mininum relative support
	 */
	private void laterLoop(IDListFatBitmap newIdList, BitSet sequencesFromIdList,
			List<BitSet> itemsetsFromIdList, int minSupport) {
		List<BitSet> itemsetIntersection = (ArrayList<BitSet>) newIdList.getItemsets();
		// We clone the sequence bitset of the current IdList
		...
		if (newIdList.getSupport() >= minSupport) {
			...
			// For each sequence appearance
			for (int i = sequenceIntersection.nextSetBit(0); i >= 0; i = sequenceIntersection
					.nextSetBit(i + 1)) {
				/*
				 * we get the itemsets of both the current Idlist and the other
				 * with which we are joining
				...
				if (thisItemset != null) {
					...
					if (greaterThanResult != null) {// We keep it if the result
													// exists
	...
	private void insertInSequence(int sid) {
		...
		int last = sid + 1;
		if (currentSize < last) {
			// We add null values in the itemset list until get the position
			// pointed by sid
	 ...
	 * 
	 * @param list
	 *            list of itemsets bitset to adjust
	 * @param length
	 *            size that the list should have
	 */
	private void setSize(List<BitSet> list, int length) {
		...
		if (dif > 0) {
			...
			// If, conversely, we have less elements, we add nulls buckets until
