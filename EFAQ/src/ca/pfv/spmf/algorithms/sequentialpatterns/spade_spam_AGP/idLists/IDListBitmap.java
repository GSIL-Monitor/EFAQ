 ...
 * is based on a hash map of entries <Integer, Bitset>, and it makes a
 * correspondence between a sid, denoted by the Integer, with a the apperances
 * of the pattern in that sequence, denoted by the bitset. In that bitset we
 * will have one bit set to 1 if in that itemset, a appearance of the pattern
 * can be found. In order to make the join operation, we will do it entry by
 * entry, for those entries shared by two sequences.
 * 
...
public class IDListBitmap implements IDList {
	 ...
	 * integer we stand for a sequence id, whereas a bitset is a representation
	 * of an itemset.
	 */
	...
	/**
	 * It adds the appearance of the pattern in the itemset "tid" and sequence
	 * "sid"
	 * 
	 * @param sid
	 *            The sequence identifier where the pattern appears
	 * @param tid
	 *            The itemset timestamp where the pattern appears
	 */
	 ...
	 * "tids" and sequence "sid"
	 * 
	 * @param sid
	 *            The sequence identifier wher the pattern appears
	 * @param tids
	 *            The set of itemset timestamps where the pattern appears
	 */
	...
	public IDList join(IDList idList, boolean equals, int minSupport) {
		...
		Map<Integer, BitSet> intersection = new HashMap<Integer, BitSet>(((IDListBitmap) idList)
				.getSecuenceItemsetEntries().size());
	 ...
	 * 
	 * @param sequenceItemsetEntries
	 *            Map where we put the new elements resulting from the join
	 *            method
	 * @param entries
	 *            Map with which we are going to join the current IdList.
	 * @param sequences
	 *            New bitset where we keep the sequences where the new IdList is
	 *            active
	 */
	private void equalLoop(Map<Integer, BitSet> sequenceItemsetEntries,
			Set<Map.Entry<Integer, BitSet>> entries, BitSet sequences) {
		// For each entry
	 ...
	 *            active
	 */
	private void laterLoop(Map<Integer, BitSet> sequenceItemsetEntries,
			Set<Map.Entry<Integer, BitSet>> entries, BitSet sequences) {
		// For each entry
		for (Map.Entry<Integer, BitSet> entry : entries) {
			// we get the bitset of the entry of the Idlist argument
	 ...
	 * 
	 * @param pattern
	 *            Pattern where we insert the sid list
	 */
	@Override
	 ...
	 * 
	 * @param sid
	 *            Sequence id where the itemsets will be inserted
	 * @param itemsets
	 *            Set of itemsets to insert in a sequence
	 */
	 ...
	 * 
	 * @param thisBitmap
	 *            Set of appearances of the the current IdList
	 * @param otherBitmap
	 *            Set of appearances of the given IdList
	...
	private BitSet shiftToLeft(BitSet bitsetArg, int temporalDistance) {
		...
		for (int bitIndex = bitsetArg.nextSetBit(0); bitIndex >= 0; bitIndex = bitsetArg
