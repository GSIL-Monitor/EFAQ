...
public class AlgoCMSPADE {
	 ...
	 * 
	 * @param support
	 *            Minimum support (from 0 up to 1)
	 * @param dfs
	 *            Flag for indicating if we want a depth first search. If false,
	 *            we indicate that we want a breath-first search.
	 * @param abstractionCreator
	 *            An abstraction creator.
	 */
	 ...
	 * 
	 * @param database
	 *            Original database in where we want to search for the frequent
	 *            patterns.
	 * @param candidateGenerator
	 *            The candidate generator used by the algorithm SPADE
	 * @param keepPatterns
	 *            Flag indicating if we want to keep the output or not
	 * @param verbose
	 *            Flag for debugging purposes
	 * @param outputFilePath
	 *            Path of the file in which we want to store the frequent
	 *            patterns. If this value is null, we keep the patterns in the
	 *            main memory. This argument is taken into account just when
	 *            keepPatterns is activated.
	 * @throws IOException
	 */
	public void runAlgorithm(SequenceDatabase database, CandidateGenerator candidateGenerator,
			boolean keepPatterns, boolean verbose, String outputFilePath) throws IOException {
	...
	public void runAlgorithmParallelized(SequenceDatabase database,
			CandidateGenerator candidateGenerator, boolean keepPatterns, boolean verbose,
		...
		// We run SPADE algorithm
		runSPADEFromSize2PatternsParallelized2(database, candidateGenerator, (long) minSupAbsolute,
				dfs, keepPatterns, verbose);
	 ...
	 *            we indicate that we want a breath-first search.
	 * @param keepPatterns
	 *            flag indicating if we are interested in keeping the output of
	 *            the algorithm
	 * @param verbose
	 *            Flag for debugging purposes
	 */
	protected void runSPADE(SequenceDatabase database, CandidateGenerator candidateGenerator,
			long minSupportCount, boolean dfs, boolean keepPatterns, boolean verbose) {
		...
		// NEW-CODE-PFV 2013
		// Map: key: item value: another item that followed the first item +
		// support
		// (could be replaced with a triangular matrix...)
		Map<Integer, Map<Integer, Integer>> coocMapAfter = new HashMap<Integer, Map<Integer, Integer>>(
				1000);
		Map<Integer, Map<Integer, Integer>> coocMapEquals = new HashMap<Integer, Map<Integer, Integer>>(
				1000);
		...
		// Inizialitation of the class that is in charge of find the frequent
		// patterns
		FrequentPatternEnumeration frequentPatternEnumeration = new FrequentPatternEnumeration(
				candidateGenerator, minSupAbsolute, saver);
		...
		frequentPatternEnumeration.execute(rootClass, dfs, keepPatterns, verbose, coocMapAfter,
				coocMapEquals);
	 ...
	 *            The set of equivalence classes from where we want to obtain
	 *            their class identifiers
	 * @return
	 */
	 ...
	 *            Flag for debugging purposes
	 */
	protected void runSPADEFromSize2PatternsParallelized(SequenceDatabase database,
			CandidateGenerator candidateGenerator, long minSupportCount, boolean dfs,
		...
		List<EquivalenceClass> size2EquivalenceClass = database
				.getSize2FrecuentSequences(minSupAbsolute);
		...
		frequentPatternEnumeration
				.setFrequentPatterns(size1Patterns.size() + size2Sequences.size());
		...
		while (frequentItems.size() > 0) {
			...
			Callable<Void> callable = new FrequentPatternEnumerationFacade(
					frequentPatternEnumeration, frequentItem, dfs, keepPatterns, verbose, saver);
		...
		try {
			int cont = 1;
			System.err.println("There are " + set.size() + " equivalence classes and "
					+ numberOfAvailableProcessors + " available processors");
			while (!set.isEmpty()) {
			...
			numberOfFrequentPatterns = frequentPatternEnumeration.getFrequentPatterns();// check
																						// the
																						// memory
																						// usage
																						// for
	 ...
	 *            Flag for debugging purposes
	 */
	protected void runSPADEFromSize2PatternsParallelized2(SequenceDatabase database,
			CandidateGenerator candidateGenerator, long minSupportCount, boolean dfs,
			boolean keepPatterns, boolean verbose) {
		frequentItems = database.frequentItems();
		Collection<Pattern> size1Sequences = getPatterns(frequentItems);
		saver.savePatterns(size1Sequences);
		List<EquivalenceClass> size2EquivalenceClasses = database
				.getSize2FrecuentSequences(minSupAbsolute);
		...
		while (frequentItems.size() > 0) {
			...
			Callable<Void> callable = new FrequentPatternEnumerationFacade(
					frequentPatternEnumeration, frequentAtom, dfs, keepPatterns, verbose, saver);
			Future<Void> future = pool.submit(callable);
			set.add(future);
			frequentItems.remove(frequentItems.size() - 1);
		...
		FrequentPatternEnumeration fpe = new FrequentPatternEnumeration(candidateGenerator,
				minSupAbsolute, saver);
