...
public class AbstractionCreator_Qualitative extends AbstractionCreator {
	...
	/**
	 * It creates an default abstraction. The abstraction is established to
	 * false
	 * 
	...
	/**
	 * It finds all the frequent 2-patterns in the original database. This
	 * method is specially useful if we think of parallelizing the execution of
	 * the Spade algorithm.
	 * 
	 * @param sequences
	 *            the list of sequences
	 * @param idListCreator
	 *            the idlist creator for creating the idlists
	 * @return the list of equivalence classes
	 */
	@Override
	public List<EquivalenceClass> getFrequentSize2Sequences(List<Sequence> sequences,
		...
		for (Sequence seq : sequences) {
			...
			for (int i = 0; i < itemsets.size(); i++) {
				...
				for (int j = 0; j < currentItemset.size(); j++) {
					...
					ItemAbstractionPair par1 = new ItemAbstractionPair(item,
							createDefaultAbstraction());
					// With the rest of items in the same itemset we make a
					// 2-sequence i-extension
					for (int k = j + 1; k < currentItemset.size(); k++) {
						...
						updateIdList(totalMap, par1, pair2, seq.getId(),
								(int) currentItemset.getTimestamp(), idListCreator);
					}
					// With the rest of items in the rest of itemsets we make a
					// 2-sequence s-extension
					for (int k = i + 1; k < itemsets.size(); k++) {
						...
						for (int n = 0; n < nextItemset.size(); n++) {
							...
							updateIdList(totalMap, par1, pair2, seq.getId(),
									(int) nextItemset.getTimestamp(), idListCreator);
						}
					}
				}
			}
	 ...
	 * 
	 * @param totalMap
	 *            Correspondences between patterns and equivalence classes
	 * @param pair1
	 *            First element of the pattern to create
	 * @param pair2
	 *            Second element of the pattern to create
	 * @param sid
	 *            Sequence identifier
	 * @param tid
	 ...
	 *            IdlistCreator
	 */
	public void updateIdList(Map<Pattern, EquivalenceClass> totalMap, ItemAbstractionPair pair1,
			ItemAbstractionPair pair2, int sid, int tid, IdListCreator idListCreator) {
		...
		for (int i = 0; i < extension.size(); i++) {
			if (i != index) {
				...
				if (index == 0) {
					...
					Abstraction_Qualitative abstractionOfRemovedPair = (Abstraction_Qualitative) extension
	...
	@Override
	public List<EquivalenceClass> getFrequentSize2Sequences(
			Map<Integer, Map<Item, List<Integer>>> database,
			Map<Item, EquivalenceClass> frequentItems, IdListCreator idListCreator) {
		Map<Pattern, EquivalenceClass> totalMap = new HashMap<Pattern, EquivalenceClass>();
		List<EquivalenceClass> result = new LinkedList<EquivalenceClass>();
		for (Entry<Integer, Map<Item, List<Integer>>> seq : database.entrySet()) {
			Integer sequence = seq.getKey();
			List<Entry<Item, List<Integer>>> itemItemsetsAssociation = new ArrayList(seq.getValue()
					.entrySet());
			// for each itemset in the sequence
				...
				for (int m = 0; m < appearances1.size(); m++) {
					...
					for (int j = 0; j < itemItemsetsAssociation.size(); j++) {
						...
						for (int k = 0; k < appearances2.size(); k++) {
							...
							if (pair2 != null) {
								updateIdList(totalMap, pair1, pair2, sequence, item2Apperance,
										idListCreator);
							}
						}
					}
