...
public class SequenceDatabase {
	 ...
	 * 
	 * @param path
	 *            Path file where the database is
	 * @param minSupport
		...
		try {
			...
			for (Item frequentItem : frequentItemsSet) {
				...
				if (equivalenceClass.getIdList().getSupport() < support) {
					...
					equivalenceClass.getIdList().setAppearingSequences(
							equivalenceClass.getClassIdentifier());
				}
			}
		...
		for (int i = beginning; i < integers.length; i++) {
			if (integers[i].codePointAt(0) == '<') { // Timestamp
				...
				if (clase == null) {
					...
					clase = new EquivalenceClass(patternCreator.createPattern(pairCreator
							.getItemAbstractionPair(item,
									abstractionCreator.createDefaultAbstraction())), idlist);
	...
	public List<EquivalenceClass> frequentItems() {
		List<EquivalenceClass> celdasDeItemsFrecuentes = new ArrayList<EquivalenceClass>(
				frequentItems.values());
		Collections.sort(celdasDeItemsFrecuentes);
		return celdasDeItemsFrecuentes;
	}

	/**
	 * It returns the map that makes the matching between items and equivalence
	 * classes.
	 * 
	 * @return the map
	 */
	...
	/**
	 * It return the equivalence classes associated with the frequent 2-Patterns
	 * that we have found
	 * 
	 * @param minSupport
	 *            Minimum absolute support
	...
	public List<EquivalenceClass> getSize2FrecuentSequences(double minSupport) {
		List<EquivalenceClass> patronesSize2 = abstractionCreator.getFrequentSize2Sequences(
				sequences, idListCreator);
	...
	private void removeInfrequentItems(List<EquivalenceClass> size2Patterns, double minSupport) {
		...
		for (int i = 0; i < size2Patterns.size(); i++) {
			Item nuevoItem = size2Patterns.get(i).getClassIdentifier().getElements().get(0)
					.getItem();
	 ...
	 * 
	 * @param keySet
	 *            the set of frequent items that should be kept.
	 */
