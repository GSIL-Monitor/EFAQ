...
public class CandidateGenerator_Qualitative implements CandidateGenerator {
	 ...
	 * 
	 * @param pattern1
	 *            The first pattern from which a new candidate is generated
	 * @param pattern2
	 *            The second pattern from which a new candidate is generated
	 * @param minSupport
	 *            The mininum relative support
	...
	public List<Pattern> generateCandidates(Pattern pattern1, Pattern pattern2, int minSupport,
			boolean doNotExploreXY, boolean doNotExploreYX, boolean doNotExploreX_Y,
			boolean doNotExploreY_X) {
		...
		if (joinBitmap.cardinality() >= minSupport) {// If that is the case
			...
			AbstractionCreator_Qualitative qualitativeCreator = AbstractionCreator_Qualitative
					.getInstance();
			 ...
			 */
			ItemAbstractionPair lastPairOfPattern1 = pattern1.getElements()
					.get(pattern1.size() - 1);
			ItemAbstractionPair lastPairOfPattern2 = pattern2.getElements()
					.get(pattern2.size() - 1);

			Abstraction_Qualitative abstractionOfLastPairOfPattern1 = ((Abstraction_Qualitative) lastPairOfPattern1
					.getAbstraction());
			Abstraction_Qualitative abstractionOfLastPairOfPattern2 = ((Abstraction_Qualitative) lastPairOfPattern2
					.getAbstraction());
			...
			if (!abstractionOfLastPairOfPattern1.hasEqualRelation()
					&& !abstractionOfLastPairOfPattern2.hasEqualRelation()) {
				// Both pattern1 and pattern2 has as last element and item
				// occurring in a different itemset to the previous one (last
				// but one)
				...
				if (!lastPairOfPattern1.getItem().equals(lastPairOfPattern2.getItem())) {
					...
					/*
					 * EXAMPLE: if the first pattern is (P < x), being x the
					 * last item, and the second pattern is (P < y), being y its
					 * last item,
					 * 
					 * We have three possible candidate patterns: C1 = (P < x <
					 * y), with x before y C2 = (P < y < x), with y before x C3
					 * = (P < (x y)), with x and y at the same time
					 */
					...
					if (pattern1.compareTo(pattern2) < 0 && doNotExploreXY == false) {
						...
						newCandidate_equalRelation.add(pairCreator.getItemAbstractionPair(
								lastPairOfPattern2.getItem(),
								qualitativeCreator.createAbstraction(true)));
					}
					else if (doNotExploreYX == false) {
						newCandidate_equalRelation = pattern2.clonePattern();
						newCandidate_equalRelation.add(pairCreator.getItemAbstractionPair(
								lastPairOfPattern1.getItem(),
								qualitativeCreator.createAbstraction(true)));
					}
					...
					if (doNotExploreY_X == false) {
						Pattern newCandidate_BeforeRelationWithChangedItems = pattern2
								.clonePattern();
			...
			else if (abstractionOfLastPairOfPattern1.hasEqualRelation()
					&& abstractionOfLastPairOfPattern2.hasEqualRelation()) {
				// We only create a candidate

				/*
				 * EXAMPLE: if the first pattern is (P x), being x the last
				 * item, and the second pattern is (P y), being y its last item,
				 * 
				 * We have a possible candidate pattern: C1 = (P x y), with x
				...
				/*
				 * if the one pattern has an equal relation and the other has
				 * not
				 */
			}
			else {
				...
				/*
				 * EXAMPLE: if the first pattern is (P x), being x the last
				 * item, and the second pattern is (P < y), being y its last
				 * item,
				 * 
				 * We have three possible candidate patterns: C1 = (P x < y),
				 * with y appearing after x
				 * 
				 * Conversely, if the first pattern is (P < x), being x the last
				 * item, and the second pattern is (P y), being y its last item,
				 * 
				 * We have three possible candidate patterns: C1 = (P y < x),
				 * with x appearing after y
				 */
	 ...
	 * 
	 * @param extension
	 *            The candidate pattern previously made from the extension of
	 *            two frequent patterns
	 * @param equivalenceClass_i
	 *            Equivalence class from the pattern1 that allowed creating the
	 *            candidate extension
	 * @param equivalenceClass_j
	 *            Equivalence class from the pattern2 that allowed creating the
	 *            candidate extension
	 * @param minSupport
	 ...
	 */
	@Override
	public IDList join(Pattern extension, EquivalenceClass equivalenceClass_i,
			EquivalenceClass equivalenceClass_j, int minSupport) {
		...
		/*
		 * We get the last pair of the identifier associated with the second
		 * Idlist,i.e. the pattern2 last pair
		 */
		ItemAbstractionPair lastPairFromEquivalenceClass_j = equivalenceClass_j
				.getClassIdentifier().getLastElement();
		...
		if (((Abstraction_Qualitative) lastPair.getAbstraction()).hasEqualRelation()) { // PFV-2013
			...
			if (!lastButOnePair.equals(lastPair)) {
				// We get the join of the IdList respecting the order given as
				// parameters
				return equivalenceClass_i.getIdList().join(equivalenceClass_j.getIdList(), true,
						minSupport);
			}
			// If the last pair of the extension pattern does not have an equal
			// relation
		}
		else { //
			// If that last pair of the extension is the same as the last pair
			// of the second Idlist
			if (lastPairFromEquivalenceClass_j.equals(lastPair)) {
				// We get the join of the IdList respecting the order given as
				// parameters
				return equivalenceClass_i.getIdList().join(equivalenceClass_j.getIdList(), false,
						minSupport);
				// Otherwise
			}
			else { // PFV-2013
				// We revert the order in the call to the join operation for
				// both IdLists
				return equivalenceClass_j.getIdList().join(equivalenceClass_i.getIdList(), false,
						minSupport);
			}
		}
