...
class CandidateGeneration {
	 ...
	 * the new set of (k)-sequences candidates. Before returning the candidate
	 * set, the algorithm prunes those canidates that cannot be frequent at all
	 * 
	 * @param frequentSet
	 *            the frequent (k-1)-sequence set, L(k-1)
	 * @param abstractionCreator
	 *            the abstraction creator
	 * @param indexationMap
	 *            a map where the frequent sequences are indexed by their first
	 *            item
	 * @param k
	 *            the number that corresponds to the current level
	 * @param minSupportAbsolute
	 *            the absolute minimum support
	 * @return the final k-candidate set
	 */
	public List<Pattern> generateCandidates(Set<Pattern> frequentSet,
			AbstractionCreator abstractionCreator, Map<Item, Set<Pattern>> indexationMap, int k,
		...
		if (k > 2) { // If we are not in the base case, i.e. we are generating a
						// level k>2
			...
			for (Pattern frequentPattern1 : frequentList) {
				...
				if (!currentItem.equals(previousItem)) {
					// We get all the patterns that starts with that item as
					// first element
				...
				if (matching != null) {
					...
					for (Pattern frequentPattern2 : matching) {
						// we try combining both frequentPattern1 and
						// frequentPattern2
						Pattern candidate = abstractionCreator.generateCandidates(
								abstractionCreator, frequentPattern1, frequentPattern2,
								minSupportAbsolute);
			...
			if (!candidateSet.isEmpty()) {
				// We prune those candidates that have some infrequent
				// subpatterns
			...
			for (int i = 0; i < frequentList.size(); i++) {
				for (int j = i; j < frequentList.size(); j++) {
					// We create candidates with all the possible combinations
					// of frequent 1-sequences
					prunedCandidates.addAll(abstractionCreator.generateSize2Candidates(
							abstractionCreator, frequentList.get(i), frequentList.get(j)));
				}
			}
		}
	 ...
	 * 
	 * @param candidateSet
	 *            the candidate k-sequence set
	 * @param frequentSet
	 *            the frequent (k-1)-sequence set
	 * @param abstractionCreator
	 * @return
	 */
	private List<Pattern> prunedSubset(List<Pattern> candidateSet, Set<Pattern> frequentSet,
		...
		for (Pattern candidate : candidateSet) {
			...
			for (int i = 0; i < candidate.getElements().size() && !isInfrequent; i++) {
				// we obtain the subpattern resulting of removing the element
				// chosen just above
				Pattern subpattern = abstractionCreator.getSubpattern(candidate, i);
				// and if this subpattern does not appear in the frequent
				// (k-1)-sequence set, L(k-1)
			...
			if (!isInfrequent) {
				// We only add in the output set those patterns that not have
				// any infrequent subpattern
