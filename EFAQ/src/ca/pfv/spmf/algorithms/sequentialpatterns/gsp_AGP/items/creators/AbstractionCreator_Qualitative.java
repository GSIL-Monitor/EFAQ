...
public class AbstractionCreator_Qualitative implements AbstractionCreator {
	 ...
	 * 
	 * @param appearingInTheSameItemset
		...
		for (Sequence s : sequences) {
			...
			for (int i = 0; i < itemsets.size(); i++) {
				...
				for (int j = 0; j < currentItemset.size(); j++) {
					...
					ItemAbstractionPair pair1 = new ItemAbstractionPair(item,
							CreateDefaultAbstraction());
					// With all of the rest of items in the itemset we make
					// 2-itemsets extensions
					...
					// And with all the items that are in later itemsets we make
					// 2-sequence extensions
	...
	/**
	 * It returns the subpattern obtained by removing an item at a given index
	 * in a pattern.
	 * 
	 * @param extension
	 *            The pattern on which we will get a subpattern
	 * @param index
	 *            the element to remove from the given pattern
	...
	public Pattern getSubpattern(Pattern extension, int index) {
		...
		PatternCreator patternCreator = PatternCreator.getInstance();
		List<ItemAbstractionPair> subpatternElements = new ArrayList<ItemAbstractionPair>(
				extension.size() - 1);
		...
		for (int i = 0; i < extension.size(); i++) {
			...
			if (i != index) {
				...
				if (i == nextIndex) {
					// If we do no have any value in "abstraction" it means that
					// it was not prepared before. In that case we take the
					// abstraction appearing in the ith-element
					if (abstraction == null) {
						abstraction = extension.getIthElement(i).getAbstraction();
					}
					// Otherwise, we use the abstraction that we had previously
					// prepared
					subpatternElements.add(pairCreator.getItemAbstractionPair(extension
							.getIthElement(i).getItem(), abstraction));
				}
				else {
					subpatternElements.add(extension.getIthElement(i));
				}
				// If we remove it, we check how are the temporal relations when
				// we do it
			}
			else {
				// If it is the first element in the pattern, we establish the
				// default abstraction for the next element
				if (index == 0) {
					abstraction = CreateDefaultAbstraction();
				}
				else {
					// If it is not the first element, we check if it appears in
					// a itemset later than its predecessor
					Abstraction_Qualitative abstractionOfRemovedElement = (Abstraction_Qualitative) extension
							.getIthElement(i).getAbstraction();
					// If that is the case, we prepare the abstraction for its
					// sucessor
	...
	public List<Pattern> createSize2Sequences(Map<Integer, Map<Item, List<Integer>>> bbdd,
		...
		for (Entry<Integer, Map<Item, List<Integer>>> seq : bbdd.entrySet()) {
			...
			List<Entry<Item, List<Integer>>> itemItemsetsAssociations = new ArrayList(seq
					.getValue().entrySet());
			// For each sequence itemset
			for (int i = 0; i < itemItemsetsAssociations.size(); i++) {
				...
				for (int m = 0; m < appearances1.size(); m++) {
					...
					ItemAbstractionPair pair1 = new ItemAbstractionPair(item1,
							CreateDefaultAbstraction());
						...
						for (int k = 0; k < appearances2.size(); k++) {
							...
							if (appearanceItem2 == appearanceItem1) {
								if (-item2.compareTo(item1) == 1) {
									pair2 = new ItemAbstractionPair(item2,
											Abstraction_Qualitative.create(true));
								}
							}
	 ...
	 * 
	 * @param sequence
	 *            The sequence where we want to find the item
	 * @param itemPair
	 *            Item to find
	 * @param absPair
	 *            abstraction of the item to find
	 * @param previousAbs
	 *            Previous abstraction in the pattern of this item
	 * @param itemsetIndex
	 *            Itemset index that the current item has
	 * @param itemIndex
	 *            Item index that the current item has.
	 * @param previousItemsetIndex
	 *            Itemset index of the previous item in the pattern
	 * @param previousItemIndex
	 *            Item index of the previous item in the pattern
	 * @return The position where the item is
	 */
	public int[] findPositionOfItemInSequence(Sequence sequence, Item itemPair,
			Abstraction_Generic absPair, Abstraction_Generic previousAbs, int itemsetIndex,
			int itemIndex, int previousItemsetIndex, int previousItemIndex) {
		Abstraction_Qualitative abs = (Abstraction_Qualitative) absPair;
		int[] pos = null;
		// If our item has an equal relation
		if (abs.hasEqualRelation()) {
			// If both itemset indices, for the current and the previous ones,
			// are the same
			...
			// If the current itemset index and the previous one point out to
			// the same position
			if (itemsetIndex == previousItemsetIndex) {
				// We make the current itemset index to point to the next
				// itemset, starting from the first item index
				itemsetIndexToSearchFor++;
				itemIndexToSearchFor = 0;
			}
			// And, finally, we look for the item in all the itemsets that
			// appear later than the original itemset index
			// pos = sequence.searchForAnItemInLaterItemset(itemPair,
			// itemsetIndexToSearchFor, itemIndexToSearchFor);
			pos = sequence.searchForTheFirstAppearance(itemsetIndexToSearchFor,
					itemIndexToSearchFor, itemPair);
		}
		return pos;
	}
	 ...
	 * item. I.e., p1 = a < (b c) < d p2 = (b c) < d < e and we want two know if
	 * the central part of both pattern is the same. If is not, we set different
	 * to true.
	 * 
	 * 
	 * 
	 * @param creator
	 * @param pattern1
	 *            The pattern that plays the role of prefix
	 * @param pattern2
	 *            The pattern that plays the rol of suffix
	 * @param minSupport
	 *            The minimum relative support
	 * @return The candidate generated from the two patterns
	 */
	public Pattern generateCandidates(AbstractionCreator creator, Pattern pattern1,
			Pattern pattern2, double minSupport) {
		...
		if (different) {// If we cannot compose any candidate we return null
			 ...
			 * otherwise, we first check if the junction of their appearing sets
			 * still has more appearances than the minimum relative support is
			 */
	 ...
	 * 
	 * @param finder
	 *            finder of candidates
	 * @param candidate
	 *            The candidate to find
	 * @param sequence
	 *            The sequence where we want to try finding the candidate
	 * @param k
	 *            The level in which we are, i.e. the candidate length
	 * @param i
	 *            the candidate item in which we start
	 * @param position
	 *            The position list for all the candiate elements
	 */
	public void isCandidateInSequence(CandidateInSequenceFinder finder, Pattern candidate,
			Sequence sequence, int k, int i, List<int[]> position) {
		finder.isCandidatePresentInTheSequence_qualitative(candidate, sequence, k, 0, position);
	}

	public List<Pattern> generateSize2Candidates(AbstractionCreator creator, Pattern pat1,
			Pattern pat2) {
		List<Pattern> output = new LinkedList<Pattern>();
		PatternCreator patternCreator = PatternCreator.getInstance();
		ItemAbstractionPairCreator pairCreator = ItemAbstractionPairCreator.getInstance();
		...
		if (!elementFromPattern1.equals(elementFromPattern2)) {
			...
			elementsOfNewPattern2.add(pairCreator.getItemAbstractionPair(
					elementFromPattern1.getItem(), Abstraction_Qualitative.create(false)));
			...
			elementsOfNewPattern3.add(pairCreator.getItemAbstractionPair(greaterPair.getItem(),
					Abstraction_Qualitative.create(true)));
