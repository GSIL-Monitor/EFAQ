...
public class AlgoGSP {
	 ...
	 *            a sequence database
	 * @param keepPatterns
	 *            flag activated if we want to keep the resulting patterns or
	 *            not
	 * @param verbose
	 *            flat activated for debugging purposes
	 ...
	 * @param supportCounter
	 *            a supportCounting element
	 * @param keepPatterns
	 *            flag activated if we want to keep the resulting patterns or
	 *            not
	 * @param verbose
	...
	protected void runGsp(SequenceDatabase database, CandidateGeneration candidateGenerator,
			SupportCounting supportCounter, boolean keepPatterns, boolean verbose)
			throws IOException {
		// we get the frequent items found in the original database
		frequentItems = database.frequentItems();
		/*
		 * And we add the sequences as the 1-level of patterns. NOTE: we need
		 * them for generating the candidates
		 */
		...
		/*
		 * And we put all those frequent 1-sequences indexed by their first
		 * item, the pattern itself, for this case
		 */
		Map<Item, Set<Pattern>> indexationMap = new HashMap<Item, Set<Pattern>>();

		// Updating the number of frequent candidates adding the number of
		// frequent items
		...
		while (frequentSet != null && !frequentSet.isEmpty()) {
			...
			candidateSet = candidateGenerator.generateCandidates(frequentSet, abstractionCreator,
					indexationMap, k, minSupAbsolute);
			frequentSet = null;
			// And we break the loop if the set of candidates is empty
			if (candidateSet == null) {
				break;
			}
			// Otherwise we continue counting the support of each candidate of
			// the set
			if (verbose) {
			...
			// We update the number of frequent patterns, adding the number
			// (k+1)-frequent patterns found
			numberOfFrequentPatterns += frequentSet.size();
			/*
			 * And we prepare the next iteration, updating the indexation map
			 * and the frequent level capable of generating the new candidates
			 */
