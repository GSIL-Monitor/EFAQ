...
class SupportCounting {
	 ...
	 *            the candidate set
	 * @param k
	 *            the level where we are checking
	 * @param minSupportAbsolute
	 *            the absolute minimum support, i.e. the minimum number of
	...
	public Set<Pattern> countSupport(List<Pattern> candidateSet, int k, double minSupportAbsolute) {
		...
		// We keep all the frequent candidates and we put them in the indexation
		// map
		...
		candidateSet = null;
		// We end returning the frequent candidates, i.e. the frequent
		// k-sequence set
		return result;
	}

	/**
	 * We check, for a sequence, if each candidate from the candidate set it
	 * appears or not
	 * 
	 * @param sequence
	 *            a sequence
	 * @param k
	 *            he level where we are checking
	 * @param candidateSet
	 *            the candidate set
	 */
	private void checkCandidateInSequence(Sequence sequence, int k, List<Pattern> candidateSet) {
		// For each candidate
		for (Pattern candidate : candidateSet) {
			// We define a list of k positions, all initialized at itemset 0,
			// item 0, i.e. first itemset, first item.
			...
			if (finder.isPresent()) {
				/*
				 * if we have a positive result, we add the sequence Id to the
				 * list of appearances associated with the candidate pattern
				 */
