...
public class CandidateInSequenceFinder {
	 ...
	 * @param sequence
	 *            the sequence where we will search for the candidate
	 * @param k
	 *            the level in which we are in the main loop of GSP, i.e. the
	 *            number of items of the candidate
	 * @param length
	 *            the current element with which we are dealing with.
	 * @param position
	 *            List of the positions of all of the elements of the candidate
	 *            in the sequence
	 */
	public void isCandidatePresentInTheSequence_qualitative(Pattern candidate, Sequence sequence,
			int k, int length, List<int[]> position) {
		...
		Abstraction_Generic previousAbstraction = length > 0 ? candidate.getIthElement(length - 1)
				.getAbstraction() : null;
		// flag to know if we should cancel the search
		boolean cancelled = false;
		// Initialization of the position of the current item, itemPair, to
		// search for
		int[] pos = null;
		// while the itemset index that correspond to the current item,
		// itempair, is withing the sequence
		while (position.get(length)[0] < sequence.size()) {
			 ...
			 */
			if (length == 0) {
				pos = sequence.searchForTheFirstAppearance(position.get(length)[0],
						position.get(length)[1], itemPair);
			}
			else {
				/*
				 * Otherwise, we find that item depending on the temporal
				 * relation between the current abstraction and the previous one
				 */
				pos = creator.findPositionOfItemInSequence(sequence, itemPair, abstractionPair,
						previousAbstraction, position.get(length)[0], position.get(length)[1],
						position.get(length - 1)[0], position.get(length - 1)[1]);
			}
			...
			if (pos != null) {
				...
				if (length + 1 < k) {
					...
					// and we keep it in the position of the following item of
					// the candidate
					position.set(length + 1, newPos);
					// And we make a recursive call in order to deal with the
					// next element of the candidate
					isCandidatePresentInTheSequence_qualitative(candidate, sequence, k, length + 1,
							position);
					...
					/*
					 * If we are in the last element of the candidate, we have
					 * checked that the candidate it appears in the sequence,
					 * and we set the corresponding flat to true
					 */
					present = true;
					return;
				}
	 ...
	 * currentItemsetIndex, currentItemIndex + 1 >. If the itemset is exceeded,
	 * we change to the next itemset, establishing the item index in 0 and so,
	 * in this case, our returning value is < currentItemsetIndex +1 , 0 >.
	 * 
	 * @param sequence
	 *            sequence which we are dealing with
	 * @param pos
	...
	/**
	 * It returns a new pair pointing out the beginning of the next itemset to
