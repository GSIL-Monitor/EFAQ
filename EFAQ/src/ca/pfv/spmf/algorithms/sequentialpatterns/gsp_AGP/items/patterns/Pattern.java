...
public class Pattern implements Comparable<Pattern> {
	 ...
	 * 
	 * @param i
	 *            ith index of the element to return
	...
	public int compareTo(Pattern arg) {
		...
		for (int i = 0; i < elementsOfSmallerPattern.size(); i++) {
			int comparison = elementsOfSmallerPattern.get(i).compareTo(
					elementsOfBiggerPattern.get(i));
			if (comparison != 0) {
				return comparison;
			}
		}
	 ...
	 * 
	 * @param p
	 *            pattern where we want to check if our current pattern is a
	 *            prefix
