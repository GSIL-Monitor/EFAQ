...
public class SequenceDatabase {
	 ...
	 * Besides, all the frequent 1-patterns are identified and the original
	 * database is updated by removing the non-frequent items
	 * 
	 * @param path
	 *            File path of the original database
	...
	public void addSequence(String[] integers) {
		...
		for (int i = start; i < integers.length; i++) {
			if (integers[i].codePointAt(0) == '<') { // Timestamp
			...
			else if (integers[i].equals("-1")) { // indica el final de un
													// itemset
			...
			else if (integers[i].equals("-2")) { // indica el final de la
													// secuencia
				...
				Pattern pattern = frequentItems.get(item);
				if (pattern == null) {
					pattern = patternCreator.createPattern(creadorPares.getItemAbstractionPair(
							item, abstractionCreator.CreateDefaultAbstraction()));
	...
	/**
	 * Reduce the original database, removing the items given in the parameter
	 * set
	 * 
	 * @param keySet
	 */
