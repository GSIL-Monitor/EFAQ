...
public class PseudoSequenceBIDE extends PseudoSequence {
	...
	int	lastItemset;	// the position where the cut was done in terms of
						// itemset in the original sequence
	int	lastItem;		// the position where the cut was done in terms of item
	 ...
	 * @param indexItemset
	 *            the position of the itemset where the pseudo sequence starts
	 *            (if it is cut at left)
	 * @param indexItem
	 *            the position of the item where the pseudo sequence starts (if
	 *            it is cut at left)
	 */
	 ...
	 * @param indexItem
	 *            the position of the item where the pseudo sequence starts (!=0
	 *            if it is cut at left)
	 * @param lastItemset
	 *            the position of the itemset where this pseudo sequence ends
	 * @param lastItem
	 *            the position of the item where this pseudo sequence ends
	 */
	protected PseudoSequenceBIDE(PseudoSequenceBIDE sequence, int indexItemset, int indexItem,
	...
	/**
	 * Constructor of a pseudo-sequence based on an original sequence
	 * (overloaded)
	 * 
	 * @param sequence
	...
	public PseudoSequenceBIDE(Sequence sequence, int indexItemset, int indexItem) {
		...
		// So we set the last itemset to the last itemset in the original
		// sequence
		// and the last item to the last item in the last itemset of the
		// original
		// sequence.
	 ...
	 * 
	 * @param indexItemset
	 *            the position of the given itemset.
	...
	// * The meaning of instance is the one from the BIDE+ article when they
	// talk about
	// * "first instance", "last instance". Here instead of finding only
	// * the "first instance" or "last instance", we find all instances.
	// */
	// protected List<PseudoSequencePair> getAllInstancesOfPrefix(List<Itemset>
	// prefix, int i){
	// List<List<Position>> listInstances
	// = getAllInstancesOfPrefixHelper(prefix, 0, new
	// ArrayList<List<Position>>(), new ArrayList<Position>(), 0);
	...
	// PseudoSequenceBIDE newSequence = new PseudoSequenceBIDE(this,
	// this.firstItemset, this.firstItem,
	...
	// protected List<List<Position>>
	// getAllInstancesOfPrefixHelper(List<Itemset> prefix, int indexItemset,
	...
	// if(listPositions.size()+ listPositionsTotal.size() ==
	// getItemOccurencesTotalCount(prefix)) // si on a trouv� tout le pr�fix
	// {
	...
	// */
	// protected PseudoSequencePair
	// getLastInstanceOfPrefixSequence(List<Itemset> prefix, int i){
	...
	// (sequenceRetour.lastItemset == sequence.lastItemset && sequence.lastItem
	// > sequenceRetour.lastItem)
	// ){
	...
	// protected PseudoSequencePair
	// getLastInstanceOfPrefixSequence(List<PseudoSequencePair> list){
	// // List<PseudoSequencePair> list = getAllInstancesOfPrefix(prefix, i);
	// // Return the last one
	// PseudoSequencePair sequenceRetourPair = list.get(0);
	// for(PseudoSequencePair sequencePair : list){
	...
	// * Method to find the first Instance of the prefix sequence X= e1, e2...
	// ei+1 in a sequence S.
	// * Exemple: first instance of AB in the sequence CAABC = CAAB.
	// * Additionnal difficulty : this sequence must respect timestamps!
	// */
	// protected PseudoSequencePair
	// getFirstInstanceOfPrefixSequence(List<Itemset> prefix, int i){
	// List<PseudoSequencePair> list = getAllInstancesOfPrefix(prefix, i);
	...
	//
	// protected PseudoSequencePair
	// getFirstInstanceOfPrefixSequence(List<PseudoSequencePair> list){
	// // List<PseudoSequencePair> list = getAllInstancesOfPrefix(prefix, i);
	// // Return the first one
	// PseudoSequencePair sequenceRetourPair = list.get(0);
	// for(PseudoSequencePair sequencePair : list){
	...
	protected Position getIthLastInLastApearanceWithRespectToPrefix(List<Itemset> prefix, int i,
		...
		if (i == getItemOccurencesTotalCount(prefix) - 1) {
			...
			Position LLiplus1 = getIthLastInLastApearanceWithRespectToPrefix(prefix, i + 1,
					lastInstancePair); // RecursiveCALL
		...
		for (int j = 0; j < size(); j++) {
			...
			if (allMatched) {
				...
				if (prefixItemsetPosition == prefix.size()) {
					PseudoSequenceBIDE newSequence = new PseudoSequenceBIDE(this,
							this.firstItemset, this.firstItem, listPositions.get(i - 1).itemset,
							listPositions.get(i - 1).item);
	...
	public PseudoSequenceBIDE getIthSemiMaximumPeriodOfAPrefix(List<Itemset> prefix, int i) {
		...
		// getFirstInstanceOfPrefixSequence(prefix,
		// getItemOccurencesTotalCount(prefix));
		PseudoSequencePair firstInstancePairNEW = getFirstInstanceOfPrefixSequenceNEW(prefix,
				getItemOccurencesTotalCount(prefix));

		Position ithlastfirst = getIthLastInFirstApearanceWithRespectToPrefix(prefix, i,
				firstInstancePairNEW);
	...
	/**
	 * Return the sum of the size of all itemsets of this sequence. Note: used
	 * by the BIDE algorithm
	 */
