...
public class Sequence {
	...
	/**
	 * indicate by how many time units a shift was performed for this sequence
	 * (used for working with timestamps based on the Hirate & Yamana
	 * algorithm).
	 */
	 ...
	 * @param sequence2
	 *            a given sequence
	 * @return Return 1 if this sequence STRICTLY contains sequence Return 2 if
	 *         this sequence is exactly the same as sequence2 Return 0 if this
	 *         sequence does not contains sequence2.
	 */
	public int strictlyContains(Sequence sequence2) {
		...
		// if it is contained
		if (retour == 2) {
			// if the size is the same, they are equal, otherwise it is strictly
			// contained
	 ...
	 *            the given sequence
	 * @param index
	 *            itemset position indicating where the comparison should start
	 *            for this sequence
	 * @param index2
	 *            itemset position indicating where the comparison should start
	 *            for the given sequence
	 * @param previousTimeStamp
	 *            previous timestamp in this sequence
	 * @param previousTimeStamp2
	 *            previous timestamp in the given sequence
	 * @return Return 1 if this sequence STRICTLY contains sequence Return 2 if
	 *         this sequence is exactly the same as sequence2 Return 0 if this
	 *         sequence does not contains sequence2.
	 */
	private int strictlyContainsHelper(Sequence sequence2, int index, int index2,
		...
		for (int i = index; i < size(); i++) {
			// calculate timestamp interval between itemset i and previous
			// itemset
			// for this sequence
			...
			// if itemset at position i contains all items from the itemset at
			// position
			// index in the given sequence and that they have the same time
			// intervals
			if (get(i).getItems().containsAll(sequence2.get(index2).getItems())
				...
				if (sequence2.size() - 1 == index2) {
					...
					int resultat = strictlyContainsHelper(sequence2, i + 1, index2 + 1, get(i)
							.getTimestamp(), sequence2.get(index2).getTimestamp());
					...
					if (resultat == 2 && sameSize) {
						...
						// otherwise, if !-0, then strictly contains and return
						// -1.
						returnValue = 1;
					}
				}
	 ...
	 * 
	 * @param mapSequenceID
	 *            a map indicating for each item (key) the IDs of sequences
	 *            containing the item (value).
	 * @param relativeMinSup
	 *            a minimum support value (double)
	 * @return a new Sequence
	 */
	public Sequence cloneSequenceMinusItems(Map<ItemSimple, Set<Integer>> mapSequenceID,
