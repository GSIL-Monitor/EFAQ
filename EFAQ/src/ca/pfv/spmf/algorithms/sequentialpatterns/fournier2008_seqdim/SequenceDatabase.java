...
public class SequenceDatabase {
	...
	/**
	 * for the Fournier08 algorithm, the cluster that was used to do the
	 * projection that results in this database.
	 */
	 ...
	 *            the input file path
	 * @throws IOException
	 *             exception if error reading file
	 */
	...
	void processSequence(String[] tokens) { //
		...
		for (String integer : tokens) {
			...
			if (integer.codePointAt(0) == '<') {
				...
				// id is the item ID and value is a value associated with the
				// item
				...
				if (indexLeftParenthesis != -1) {
					...
					int value = Integer.parseInt(integer.substring(indexLeftParenthesis + 1,
							indexRightParenthesis));
