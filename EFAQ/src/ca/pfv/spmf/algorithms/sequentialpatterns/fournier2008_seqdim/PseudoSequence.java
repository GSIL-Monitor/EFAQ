...
class PseudoSequence {
	...
	// this represents how much time unit have been removed when the sequence is
	// cut at left
	private long		timeShift;
	// this is the original sequence
	private Sequence	sequence;
	// the following variables indicates at which itemset indexes and item
	// indexes
	// the sequence has been cut at left and right in terms of the original
	// sequence
	 ...
	 * 
	 * @param timeShift
	 *            how much time unit have been removed when the pseudo-sequence
	 *            is cut at left
	 * @param sequence
	 *            the original pseudo-sequence
	 * @param indexItemset
	 *            the first itemset index
	 * @param indexItem
	...
	public PseudoSequence(long timeShift, PseudoSequence sequence, int indexItemset, int indexItem) {
		...
		if (this.firstItemset == sequence.firstItemset) {
			// then the first item position is the sum of the current position
			// with
	...
	/**
	 * Constructor to build a new pseudo-sequence based on a pseudo-sequence,
	 * and cut at left and right.
	 * 
	 * @param timeShift
	 *            how much time unit have been removed when the pseudo-sequence
	 *            is cut at left
	 ...
	 *            the first item index
	 * @param lastItemset
	 *            the last itemset index
	 * @param lastItem
	 *            the last item index
	 */
	public PseudoSequence(long timeShift, PseudoSequence sequence, int indexItemset, int indexItem,
			int lastItemset, int lastItem) {
		// save the parameters
		this.timeShift = timeShift;
		this.sequence = sequence.sequence;
		// the first itemset is expressed in terms of the first itemset
		...
		if (this.firstItemset == sequence.firstItemset) {
			// add the first item position to the first item position in
			// original sequence
			this.firstItem = indexItem + sequence.firstItem;
		}
		else {
			// otherwise, the first item position is the item position received
			// as parameter
			this.firstItem = indexItem;
		}
	 ...
	 * 
	 * @param timeShift
	 *            the number of time units removed from the original sequence
	 * @param sequence
	 *            the original sequence
	 * @param indexItemset
	 *            where the pseudo sequence starts in terms of itemset index
	 * @param indexItem
	 *            where the pseudo sequence starts in terms of item index in the
	 *            itemset where the pseudosequence starts.
	 */
	public PseudoSequence(long timeShift, Sequence sequence, int indexItemset, int indexItem) {
		...
		this.firstItem = indexItem;
		// Last itemset and item (by default, this is the last item & itemset of
		// the sequence.
	...
	public ItemSimple getItemAtInItemsetAt(int indexItem, int indexItemset) {
		...
		if (isFirstItemset(indexItemset)) {
			...
			// otherwise, just return the item at the given position in this
			// itemset
	...
	/**
	 * Get the timestamp associated to a given itemset without considering the
	 * time shift.
	 * 
	 * @param indexItemset
	 *            the index of the itemset.
	...
	/**
	 * Internal class representing a position in a pseudo-sequence. (position of
	 * an itemset + position of an item).
	 */
	...
	/**
	 * Method that find all instances of a prefix in a sequence S. The meaning
	 * of instance is the one from the BIDE+ article when they tak about
	 * "first instance", "last instance". Here instead of finding only the
	 * "first instance" or "last instance", we find all instances and they also
	 * respect the timestamps!
	 */
	List<PseudoSequencePair> getAllInstancesOfPrefix(Sequence prefix, int i) {
		List<List<Position>> listInstances = getAllInstancesOfPrefixHelper(prefix, 0,
				new ArrayList<List<Position>>(), new ArrayList<Position>(), 0, 0);
		...
		for (List<Position> listPositions : listInstances) {
			PseudoSequence newSequence = new PseudoSequence(0, this, this.firstItemset,
					this.firstItem, listPositions.get(i - 1).itemset, listPositions.get(i - 1).item);
	...
	List<List<Position>> getAllInstancesOfPrefixHelper(Sequence prefix, int indexItemset,
		...
		for (int i = decalageItemset; i < size(); i++) {
			...
			if (!firstTime
					&& getTimeStamp(i) - itemsetShift != prefix.get(indexItemset).getTimestamp()) { // V�RIFIER
																									// D�CALAGE
																									// TEMPS
				continue;
			}
			...
			for (int j = 0; j < getSizeOfItemsetAt(i); j++) {
				...
				if (id == iDCourant) {// l'item match
					...
					if (listPositions.size() + listPositionsTotal.size() == prefix
							.getItemOccurencesTotalCount()) // si on a trouv�
															// tout le pr�fix
					{
						...
						if (indexItemset + 1 < prefix.size()) {
							getAllInstancesOfPrefixHelper(prefix, indexItemset + 1, allInstances,
									newList, decalage, i + 1);
						}
					}
					else {
						indexItem++;
	...
	/**
	 * Last instance of a prefix sequence X in a sequence S. For example, the
	 * last instance of AB in ABBCA is ABB. Additionnal difficulty : this
	 * sequence must respect timestamps!
	 */
	 ...
	 * Instance of the prefix sequence X= e1, e2... ei+1 in a sequence S.
	 * Exemple: first instance of AB in the sequence CAABC = CAAB. Additionnal
	 * difficulty : this sequence must respect timestamps!
	 */
	 ...
	 * n = size of S If i == n, it is the last appearance of ei in the last
	 * instance of Sp. If 0 <= i < n, it is the last appearance of ei in the
	 * last instance of Sp, and LLi must appear before LLi+1. Example : If S=
	 * CAABC and SP = AB then LL0 = second A in CAABC If S= CACAC and SP = CAC
	 * then LL0 = second C in S, LL1 = second A in S and LL2 = third C in S
	 * 
	 * @param prefix
	 *            : le prefix
	 * @param i
	 *            : le i�me �l�ment du pr�fixe.
	 * @return the ith last-in-last appearance
	 */
	Position getIthLastInLastApearanceWithRespectToPrefix(Sequence prefix, int i,
		...
		// - the list of positions for each element of prefix in that last
		// instance.
		PseudoSequencePair lastInstancePair = getLastInstanceOfPrefixSequence(prefix,
				prefix.getItemOccurencesTotalCount());

		// IF WE DON'T USE TIMESTAMP THE "ITH LAST IN LAST" IS A LITTLE BIT
		// COMPLICATED TO GET :
	...
	/**
	 * Get the ith maximum period of a prefix sequence for this sequence S. The
	 * ith maximum period of the prefix Sp in S is : * if 0 < i <= n, it is the
	 * piece of sequence between the end of the first instance of prefix e1...
	 * ei-1 in S and the ith last-in-last appearance with respect to prefix Sp *
	 * if i = 0, it is the piece of sequence in S located before the first
	 * last-in-last appearance with respect to prefix Sp. Example1: if S = ABCB
	 * and Sp = AB the 1th semi-period of Sp in S is empty the 2th semi-period
	 * of Sp in S is BC Example2: if S = ABBB and Sp = BB the 1th semi-period of
	 * Sp in S is AB the 2th semi-period of Sp in S is B
	 */
	PseudoSequence getIthMaximumPeriodOfAPrefix(Sequence prefix, int i, boolean withTimeStamps) {
		if (i == 0) { // it is the piece of sequence in S located before the
						// first last-in-last appearance with respect to prefix
						// Sp.
		...
		// ELSE it is the piece of sequence between the end of the first
		// instance of prefix e1... ei-1 in S
	...
	List<PseudoSequence> getAllIthMaxPeriodOfAPrefix(Sequence prefix, int i, boolean b) {
		if (i == 0) {
			...
			for (PseudoSequencePair instance : getAllInstancesOfPrefix(prefix,
					prefix.getItemOccurencesTotalCount())) {
		...
		for (PseudoSequencePair instance : getAllInstancesOfPrefix(prefix, i)) {
			PseudoSequence period = trimBeginingAndEnd(instance.list.get(i - 1),
					instance.list.get(i));
			periods.add(period);
		}
		return periods;
	...
	/**
	 * Method that is used by the one above. This method cut a sequence by
	 * removing some part at the begining and at the end. IMPORTANT : this
	 * method assumes that the sequence has never been cut. This simplify what
	 * this method has to do to handle time.
	 * 
	...
	long getTimeSucessor() {
		...
		else if (absolutePositionLastElement < sequence.size() - 1) { // if the
																		// last
																		// itemset
																		// is
																		// not
		...
		return 0; // if it is the last itemset of the original sequence and it
					// was not cut.
	}
	...
	/**
	 * Get the ith first-in-last appearance with respect to a prefix sequence
	 * Sp. n = size of S If i == n, it is the last appearance of ei in the first
	 * instance of Sp. If 0 <= i < n, it is the last appearance of ei in the
	 * first instance of Sp, and LFi must appear before LFi+1.
	 * 
	 * @param prefix
	 *            : the prefix
	 * @param i
	 *            : the i�me element of the prefix.
	 * @return the ith first-in-last appearance.
	 */
	Position getIthLastInFirstApearanceWithRespectToPrefix(Sequence prefix, int i,
			boolean withTimestamps) {
		...
		// - the list of positions for each element of prefix in that first
		// instance.
	 ...
	 * The ith semi-maximum period of the prefix Sp in S is : * if 0 < i <= n,
	 * it is the piece of sequence between the end of the first instance of
	 * prefix e1... ei-1 in S and the ith last-in-first appearance with respect
	 * to prefix Sp * if i = 0, it is the piece of sequence in S located before
	 * the first last-in-first appearance with respect to prefix Sp.
	 */
	PseudoSequence getIthSemiMaximumPeriodOfAPrefix(Sequence prefix, int i, boolean withTimestamps) {

		if (i == 0) { // it is the piece of sequence in S located before the
						// first last-in-first appearance with respect to prefix
						// Sp.
		...
		// / Important: we have to cut the prefix at ei-1 (short prefix = e1 ...
		// ei-1).
		...
		PseudoSequencePair firstInstance = this.getFirstInstanceOfPrefixSequence(prefix, i); // e1...
																								// ei-1
		...
		Position ithlastfirst = this.getIthLastInFirstApearanceWithRespectToPrefix(prefix, i,
				withTimestamps);
		// we return the piece of sequence between the end of the first instance
		// of prefix e1... ei-1 in S
		// and the ith last-in-first appearance with respect to prefix Sp
