...
public class AlgoSeqDim {
	...
	private long			startTime;														// the
																							// start
																							// time
																							// of
																							// the
																							// algorithm
	private long			endTime;														// the
																							// end
																							// time
																							// of
																							// the
																							// algorithm
	private boolean			mineClosedPatterns	= false;									// if
																							// true,
																							// only
																							// closed
																							// patterns
	 ...
	 * 
	 * @param database
	 *            and MDSequence database
	 * @param algoPrefixSpan
	 *            a prefixpsan based algorithm (BIDE, PrefixSpan or Fournier08)
	 * @param algoDim
	 *            an instance of the DIM algorithm
	 * @param mineClosedPatterns
	 *            if true, only closed mdsequential pattern will be returned
	 * @param output
	 *            a path for writting the result to an output file
	 * @return the set of MD-sequential patterns found
	 * @throws IOException
	 *             exception if error writing to file
	 */
	public MDSequences runAlgorithm(MDSequenceDatabase database,
			AbstractAlgoPrefixSpan algoPrefixSpan, AlgoDim algoDim, boolean mineClosedPatterns,
	 ...
	 * @param minsupp
	 *            the minsup threshold (double)
	 * @param algoDim
	 *            an instance of the DIM algorithm
	 * @throws IOException
	 *             exception if error writing output file
	 */
	private void trySequence(Sequence sequence, MDSequenceDatabase database, double minsupp,
			AlgoDim algoDim) throws IOException {
	 ...
	 * 
	 * @param sequence
	 *            the sequence in the md seq. pattern
	 * @param mdsequence
	 *            the md seq. pattern
	 * @throws IOException
	 *             exception if error writing to file
	...
	private MDPatternsDatabase createProjectedDatabase(Set<Integer> patternsIds,
			MDPatternsDatabase patternsDatabase) {
	 ...
	 * 
	 * @param databaseSize
	 *            the number of MDsequences in the original database.
	 */
	 ...
	 * mining + closed itemset mining != closed multi-dim seq. patt. mining. For
	 * more details about this, see the paper published by Panida Songram, Veera
	 * Boonjing and Sarun Intakosum (2006) that explains why we can do that.
	 * 
	 * @throws IOException
	...
	private void removeRedundancy() throws IOException {
		...
		for (int i = sequences.getLevels().size() - 1; i > 0; i--) {
			...
			for (MDSequence sequence : sequences.getLevel(i)) {
				...
				for (int j = i; j < sequences.getLevels().size() && !included; j++) {
					...
					for (MDSequence sequence2 : sequences.getLevel(j)) {
						...
						if (sequence != sequence2
								&& sequence2.getAbsoluteSupport() == sequence.getAbsoluteSupport()
				...
				// if the md sequential pattern is NOT included in another
				// sequential pattern
