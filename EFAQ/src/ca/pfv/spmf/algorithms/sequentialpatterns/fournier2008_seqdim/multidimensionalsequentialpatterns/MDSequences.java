...
public class MDSequences {
	...
	// size i.
	private final List<List<MDSequence>>	levels			= new ArrayList<List<MDSequence>>();	// itemset
																									// sorted
																									// by
																									// size
	...
	public MDSequences(String name) {
		this.name = name;
		levels.add(new ArrayList<MDSequence>()); // the level 0 is empty by
													// default.
	}
	 ...
	 * 
	 * @param sequenceCount
	 *            the number of MDSequences in the MDSequences database.
	 */
		...
		for (List<MDSequence> level : levels) {
			...
			for (MDSequence sequence : level) {
				...
				System.out.print("support :  "
						+ sequence.getFormattedRelativeSupport(sequenceCount));
