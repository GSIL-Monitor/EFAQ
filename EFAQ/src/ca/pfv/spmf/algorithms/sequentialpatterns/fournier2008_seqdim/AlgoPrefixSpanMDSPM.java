...
public class AlgoPrefixSpanMDSPM extends AbstractAlgoPrefixSpan {
	...
	// for statistics
	private long			startTime;			// start time of the latest
												// algorithm execution
	private long			endTime;			// end time of the latest
												// algorithm exeuction
	...
	private void prefixSpan(SequenceDatabase database) {
		...
		for (Entry<ItemSimple, Set<Integer>> entry : mapSequenceID.entrySet()) {
			...
			if (entry.getValue().size() >= minsuppRelative) {
				...
				PseudoSequenceDatabase projectedDatabase = buildProjectedContext(item,
	...
	private Map<ItemSimple, Set<Integer>> calculateSupportOfItems(SequenceDatabase database) {
		// Use a set to remember which item were seen already from a sequence
		// when scanning
		// a sequence
		Set<Integer> alreadyCounted = new HashSet<Integer>();
		// the last sequence scanned
		Sequence lastSequence = null;
		// the map for storing association from each item (key) to the set of
		// IDs of sequences (value)
		// containing the item.
			...
			for (Itemset itemset : sequence.getItemsets()) {
				...
				for (ItemSimple item : itemset.getItems()) {
					...
					if (!alreadyCounted.contains(item.getId())) {
						// get the set of sequence IDs containing this item
						// until now
						...
						// remember that we have seen this item already in this
						// sequence
	 ...
	 */
	private PseudoSequenceDatabase buildProjectedContext(ItemSimple item,
			...
			for (int i = 0; i < sequence.size(); i++) {
				...
				if (index != -1 && sequence.isCutAtLeft(i) == inSuffix) {
					...
					if (index != sequence.getSizeOfItemsetAt(i) - 1) {
						...
						// the item does not appear in an itemset that is a
						// postfix

						// create a new pseudo-sequence by cutting right after
						// the item
						// used for the projection
						PseudoSequence newSequence = new PseudoSequence(
	...
	/**
	 * This is the recursive method for growing a prefix of size >=1 to find
	 * larger sequential patterns
	 * 
	 * @param prefix
	 *            the prefix
	 * @param k
	 *            the size of the current prefix + 1
	 * @param database
	...
	private void recursion(Sequence prefix, int k, PseudoSequenceDatabase database) {
		...
		for (Pair paire : pairs) {
			...
			if (paire.getCount() >= minsuppRelative) {
				...
				PseudoSequenceDatabase projectedContext = buildProjectedContext(paire.getItem(),
						database, paire.isPostfix());
	 ...
	 * pair is an item ID and a boolean indicating if the item appear in a
	 * postfix or not. This is for k> 1.
	 * 
	 * @param sequences
	 *            the sequence database
		...
		for (PseudoSequence sequence : sequences) {
			...
			for (int i = 0; i < sequence.size(); i++) {
				...
				for (int j = 0; j < sequence.getSizeOfItemsetAt(i); j++) {
					...
					// create the pair corresponding to this item in this
					// itemset
					...
					if (!alreadyCountedForSequenceID.contains(pair)) {
						...
						// add the pair to the set of pairs already seen for
						// this sequence.
	 ...
	 * 
	 * @param databaseSize
	 *            the number of sequences in the original sequence database.
	 */
