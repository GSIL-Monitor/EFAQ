...
/**
 * This class is for creating a triangular matrix of integers. All the elements
 * in the matrix are initialized to zero and can be changed to other integer
...
public class TriangularMatrix {
	 ...
	 * 
	 * @param elementCount
	 *            the desired number of lines in the matrix.
	 */
	public TriangularMatrix(int elementCount) {
		// save the number of lines
		this.elementCount = elementCount;
		// initialize the matrix
		matrix = new int[elementCount - 1][]; // -1 cause we want it shorter of
												// 1 element
		for (int i = 0; i < elementCount - 1; i++) { // -1 cause we want it
														// shorter of 1 element
	...
	public int getSupportForItems(int i, int j) {
		if (j < i) {
			return getSupportForItems(j, i); // so that id is always smaller
												// than id2
		}
		else {
