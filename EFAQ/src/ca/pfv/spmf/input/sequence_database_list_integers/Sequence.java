...
public class Sequence {
	...
	/**
	 * Make a copy of this sequence while removing some items that are
	 * infrequent with respect to a threshold minsup.
	 * 
	 * @param mapSequenceID
	 *            a map with key = item value = a set of sequence ids containing
	 *            this item
	 * @param relativeMinSup
	 *            the minimum support threshold chosen by the user.
	 * @return a copy of this sequence except that item(s) with a support lower
	 *         than minsup have been excluded.
	 */
	public Sequence cloneSequenceMinusItems(Map<Integer, Set<Integer>> mapSequenceID,
		...
		for (List<Integer> itemset : itemsets) {
			...
			List<Integer> newItemset = cloneItemsetMinusItems(itemset, mapSequenceID,
					relativeMinSup);
	 ...
	 * 
	 * @param mapSequenceID
	 *            a map with key = item value = a set of sequence containing
	 *            this item
	 * @param relativeMinSup
	 *            the minimum support threshold chosen by the user.
	 * @return a copy of this sequence except that item(s) with a support lower
	...
	public Sequence cloneSequenceMinusItems(double relativeMinSup,
		...
		for (List<Integer> itemset : itemsets) {
			// call a method to copy this itemset
			List<Integer> newItemset = cloneItemsetMinusItems(relativeMinSup, itemset,
					mapSequenceID);
			// add the copy to the new sequence
			if (newItemset.size() != 0) {
				sequence.addItemset(newItemset);
	...
	/**
	 * Make a copy of an itemset while removing some items that are infrequent
	 ...
	 * @param itemset
	 *            the itemset
	 * @return a copy of this itemset except that item(s) with a support lower
	 *         than minsup have been excluded.
	 */
	 ...
	 *         than minsup have been excluded.
	 */
	public List<Integer> cloneItemsetMinusItems(List<Integer> itemset,
			Map<Integer, Set<Integer>> mapSequenceID, double minSupportAbsolute) {
		// create a new itemset
		List<Integer> newItemset = new ArrayList<Integer>();
		// for each item of the original itemset
		for (Integer item : itemset) {
			...
			// if this set is not null (an infrequent item) and the support is
			// higher than minsup...
