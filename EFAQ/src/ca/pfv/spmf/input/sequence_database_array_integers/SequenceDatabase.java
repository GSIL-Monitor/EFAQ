...
public class SequenceDatabase {
	 ...
	 * 
	 * @param tokens
	 *            A list of tokens from the line (which were separated by spaces
	...
	public void addSequence(String[] tokens) {
		...
		for (String token : tokens) {
			...
			// if the token is -1, it means that we reached the end of an
			// itemset.
	...
	// std += Math.abs((meansizeItems - elementList) /
	// ((float)sequences.size()));
	// }
	// std = Math.sqrt(std);
	//
