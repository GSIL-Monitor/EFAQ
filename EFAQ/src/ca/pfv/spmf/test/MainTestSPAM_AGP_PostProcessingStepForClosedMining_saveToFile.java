...
/**
 * Example of how to use the algorithm SPAM but executing a postprocessing step
 * at the end, in order to find only the closed frequent patterns. The output is
...
public class MainTestSPAM_AGP_PostProcessingStepForClosedMining_saveToFile {
	...
	public static void main(String[] args) throws IOException {
		...
		// sequenceDatabase.loadFile(fileToPath("ExampleClaSP.txt"), support);
		double relativeSupport = sequenceDatabase.loadFile(fileToPath("contextPrefixSpan.txt"),
				support);
		// double relativeSupport =
		// sequenceDatabase.loadFile(fileToPath("gazelle.txt"), support);

		AlgoClaSP algorithm = new AlgoClaSP(relativeSupport, abstractionCreator,
