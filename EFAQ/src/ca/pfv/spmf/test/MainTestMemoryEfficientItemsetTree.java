...
public class MainTestMemoryEfficientItemsetTree {
	...
	public static void main(String[] arg) throws IOException {
		...
		// print the statistics about the tree construction time and print the
		// tree in the console
		...
		//
		// // It is also possible to add transactions manually. For example, we
		// can add
		...
		//
		// // Example query 2: get all itemsets that subsume an itemset (e.g. {1
		// 2}) and their support
		...
		//
		// // Example query 3: get all itemsets that subsume an itemset and have
		// a support higher than minsup
		...
		for (List<Itemset> list : result2.table) {
			if (list != null) {
				for (Itemset itemset : list) {
					...
					// checkResult(itemset); // THIS IS FOR DEBUGING ONLY,
					// IGNORE THIS LINE
				}
			}
		}
		//
		// // Example query 4 : generate all association rules with an itemset
		// as antecedent and minsup and minconf
		System.out
				.println("EXAMPLE QUERY: FIND ALL ASSOCIATION RULE WITH AN ITEMSET {1} AS ANTECEDENT AND MINSUP >= 2 and minconf >= 0.1");
		minsup = 2;
		double minconf = 0.1;
